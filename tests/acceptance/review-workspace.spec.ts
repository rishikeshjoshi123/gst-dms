...
test('exact reference relationship stays non-effective until one Review or Timeline decision', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 740 })
  await page.emulateMedia({ colorScheme: 'dark', reducedMotion: 'reduce' })
  await page.goto('/login')
  await page.getByLabel('Email address').fill('reference-race@example.test')
  await page.getByLabel('Password').fill('RelationshipFixture173!')
  await page.getByRole('button', { name: 'Sign in' }).click()
  await expect(page).toHaveURL(/\/dashboard$/)
  await page.goto('/review?type=relationship_suggestion')
  await page.getByRole('button', { name: 'Review exact reference relationship' }).click()
  await expect(page.getByText('Exact reference in the current PDF')).toBeVisible()
  await expect(page.getByText(/proposes only refers_to/)).toBeVisible()
  await expect(page.getByText(/not a Timeline progression edge/)).toBeVisible()
  const exactSource = page.getByRole('link', { name: 'Open exact PDF source · Page 1' })
  await expect(exactSource).toHaveAttribute('href', /\/documents\/151e0000.*version=15100000.*page=1/)
  ...
  await page.goBack()
  await expect(page.getByRole('heading', { name: 'Review exact reference relationship' })).toBeVisible()
  await page.getByRole('tab', { name: 'Evidence' }).focus()
  await page.keyboard.press('ArrowRight')
  await expect(page.getByRole('tab', { name: 'Decision' })).toBeFocused()
  await expect(page.getByRole('radio', { name: /Accept source → target refers to/ })).toBeVisible()
  await page.getByRole('tab', { name: 'Evidence' }).click()
  await noOverflow(page)

  await page.setViewportSize({ width: 1440, height: 900 })
  await page.emulateMedia({ colorScheme: 'light', reducedMotion: 'no-preference' })
  await page.getByRole('link', { name: 'Review on timeline' }).click()
  await expect(page).toHaveURL(/\/matters\/151d0000-0000-0000-0000-000000000099\?.*reviewItem=/)
  await expect(page.getByRole('heading', { name: 'Review exact reference' })).toBeVisible()
  await expect(page.getByText('Dashed edge is a candidate only · no relationship is effective')).toBeVisible()
  await expect(page.getByText('Candidate · refers to')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Add relationship' })).toHaveCount(0)
  await expect(page.getByText(/Accepted refers_to is inspectable but is not a Timeline progression edge/)).toBeVisible()

  await page.setViewportSize({ width: 320, height: 740 })
  await page.emulateMedia({ colorScheme: 'dark', reducedMotion: 'reduce' })
  await expect(page.getByRole('heading', { name: 'Review exact reference' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Add relationship' })).toHaveCount(0)
  const chronologyReviewUrl = new URL(page.url())
  chronologyReviewUrl.searchParams.set('view', 'chronology')
  chronologyReviewUrl.searchParams.set('document', '151e0000-0000-0000-0000-000000000001')
  chronologyReviewUrl.searchParams.set('inspector', 'relationships')
  await page.goto(chronologyReviewUrl.toString())
  await expect(page.getByRole('heading', { name: 'Review exact reference' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Add relationship' })).toHaveCount(0)
  await expect(page.getByRole('button', { name: /Archive relationship/ })).toHaveCount(0)
  await page.getByRole('radio', { name: 'Accept exact refers_to' }).check()
  await page.getByRole('button', { name: 'Review relationship decision' }).click()
  await expect(page.getByRole('dialog')).toContainText('not a Timeline progression edge')
  await page.getByRole('button', { name: 'Return to decision' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  expect(db("SELECT count(*) FROM public.relationship_suggestion_decisions;").trim()).toBe('0')
  await page.getByRole('button', { name: 'Review relationship decision' }).click()
  await page.getByLabel('Reason', { exact: true }).fill('The exact current source supports the limited refers to relationship.')
  await page.getByRole('button', { name: 'Accept relationship' }).click()
  await expect(page.getByText('The exact refers_to relationship was accepted. It remains inspectable and is not a Timeline progression edge.', { exact: true })).toBeVisible()
  await noOverflow(page)
  await page.getByRole('link', { name: 'Return to Review' }).click()
  await expect(page).toHaveURL(/\/review\?type=relationship_suggestion.*item=/)
  await expect(page.getByRole('heading', { name: 'Review exact reference relationship' })).toBeVisible()
  await page.getByRole('button', { name: 'View decision' }).click()
  await expect(page.getByText('Relationship decision recorded')).toBeVisible()
})
