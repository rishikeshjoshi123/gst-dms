-- A Matter's procedural status (for example, tribunal) is not its lifecycle.
-- Keep assignment eligibility on the existing active-record and client fences.
BEGIN;
...
BEGIN
  ...
  IF NOT EXISTS(
    ...
    WHERE matter.id=p_matter_id AND matter.org_id=x.org_id
...
COMMENT ON FUNCTION public.assign_intake_to_new_document(uuid,uuid,text,uuid,uuid) IS
  'Authenticated Intake assignment preserving original uploader document/version provenance while recording the assigning actor separately; Matter eligibility uses lifecycle state, not procedural status.';
