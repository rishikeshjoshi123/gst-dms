...
BEGIN
  ...
  rejected:=false; BEGIN PERFORM public.materialize_verified_source_field_candidate(run_a,'tax_period:invalid-printed-fy','document.tax_period','structured',jsonb_set(period,'{printed_financial_years}','["2023-99"]'),1,'Tax period JAN 2020',NULL,0.9,'invalid',ARRAY['fixture'],NULL); EXCEPTION WHEN others THEN rejected:=true; END;
  IF NOT rejected THEN RAISE EXCEPTION 'semantically invalid printed financial year was accepted'; END IF;
  ...
  PERFORM public.materialize_verified_source_field_candidate(run_a,'official_reference:unicode-parity','document.official_reference.self_identifier','structured',self_ref||'{"raw":"ＳＣＮ ‐ ٠٠١ / ２０２６","display":"SCN-٠٠١/2026","normalized_value":"SCN-٠٠١/2026","components":{"kind":"notice_reference","segments":["SCN","٠٠١","2026"],"prefix":"SCN","numericCore":"2026","year":"2026"}}',1,'Notice SCN / 01',NULL,0.9,'invalid',ARRAY['fixture'],NULL);
