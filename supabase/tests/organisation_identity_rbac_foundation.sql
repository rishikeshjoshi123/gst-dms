-- Run only against a disposable local Supabase database after migration 00029.
...
BEGIN
  ...
  IF (SELECT capabilities FROM public.get_my_organisation_context()) <> ARRAY[
    'team.view', 'team.invite.standard', 'team.role.manage_standard',
    'team.membership.suspend_standard', 'organisation.profile.manage', 'organisation.operations.manage', 'trash.purge'
  ...
  IF (SELECT capabilities FROM public.get_my_organisation_context()) <> ARRAY['team.view']::text[] THEN
  ...
  IF (SELECT capabilities FROM public.get_my_team_members()
      ...
      <> ARRAY['team.view']::text[] THEN RAISE EXCEPTION 'viewer target capability projection failed'; END IF;
