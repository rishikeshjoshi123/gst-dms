...
DO $$ DECLARE r record; target uuid; BEGIN
 ...
 SELECT * INTO r FROM public.suspend_standard_organisation_member(target,1,'Viewer cannot suspend','return_open_tasks_to_team','16120000-0000-4000-8000-000000000001'); IF r.code<>'not_allowed' THEN RAISE EXCEPTION 'viewer command allowed'; END IF;
END $$;
SELECT set_config('request.jwt.claim.sub','a0010000-0000-0000-0000-000000000005',true);
DO $$ DECLARE r record; target uuid:=current_setting('casechain.target_associate_membership')::uuid; BEGIN SELECT * INTO r FROM public.suspend_standard_organisation_member(target,1,'Suspended cannot suspend','return_open_tasks_to_team','16120000-0000-4000-8000-000000000002'); IF r.code<>'not_allowed' THEN RAISE EXCEPTION 'suspended command allowed'; END IF; END $$;
...
DO $$ DECLARE r record; own_id uuid; owner_id uuid; foreign_id uuid; target uuid; rev bigint; BEGIN
 ...
 target:=current_setting('casechain.target_associate_membership')::uuid; SELECT target_revision INTO rev FROM public.get_standard_member_suspension_impact(target);
 SELECT * INTO r FROM public.suspend_standard_organisation_member(own_id,1,'Self denied','return_open_tasks_to_team','16120000-0000-4000-8000-000000000003'); IF r.code<>'not_available' THEN RAISE EXCEPTION 'self allowed'; END IF;
 SELECT * INTO r FROM public.suspend_standard_organisation_member(owner_id,1,'Owner denied','return_open_tasks_to_team','16120000-0000-4000-8000-000000000004'); IF r.code<>'not_available' THEN RAISE EXCEPTION 'owner allowed'; END IF;
 SELECT * INTO r FROM public.suspend_standard_organisation_member(foreign_id,1,'Foreign denied','return_open_tasks_to_team','16120000-0000-4000-8000-000000000005'); IF r.code<>'not_available' THEN RAISE EXCEPTION 'foreign allowed'; END IF;
 SELECT * INTO r FROM public.suspend_standard_organisation_member(target,rev+1,'Stale denied','return_open_tasks_to_team','16120000-0000-4000-8000-000000000006'); IF r.code<>'conflict' THEN RAISE EXCEPTION 'stale accepted'; END IF;
END $$;
...
DO $$ DECLARE impact record; result record; target uuid; rev bigint; BEGIN
 ...
 IF impact.code<>'ok' OR impact.open_task_count<>2 OR impact.task_disposition<>'return_open_tasks_to_team'
 ...
 SELECT * INTO result FROM public.suspend_standard_organisation_member(target,rev,'Security access review','return_open_tasks_to_team','16120000-0000-4000-8000-000000000010');
 IF result.code<>'suspended' OR result.returned_task_count<>2 OR result.replayed THEN RAISE EXCEPTION 'owner suspension failed'; END IF;
 SELECT * INTO result FROM public.suspend_standard_organisation_member(target,rev,'Security access review','return_open_tasks_to_team','16120000-0000-4000-8000-000000000010');
 IF result.code<>'suspended' OR NOT result.replayed THEN RAISE EXCEPTION 'replay failed'; END IF;
 SELECT * INTO result FROM public.suspend_standard_organisation_member(target,rev,'Changed reason','return_open_tasks_to_team','16120000-0000-4000-8000-000000000010');
...
SELECT set_config('request.jwt.claim.sub','a0010000-0000-0000-0000-000000000004',true);
DO $$ DECLARE result record; target uuid; rev bigint; BEGIN
 target:=current_setting('casechain.target_viewer_membership')::uuid; SELECT target_revision INTO rev FROM public.get_standard_member_suspension_impact(target);
 SELECT * INTO result FROM public.suspend_standard_organisation_member(target,rev,'Administrative access review','return_open_tasks_to_team','16120000-0000-4000-8000-000000000011');
 IF result.code<>'suspended' OR result.returned_task_count<>0 THEN RAISE EXCEPTION 'admin suspension failed'; END IF;
END $$;

RESET ROLE;
DO $$ BEGIN
 IF (SELECT count(*) FROM public.tasks WHERE id IN ('16100000-0000-4000-8000-000000000001','16100000-0000-4000-8000-000000000002') AND assignee_user_id IS NULL AND revision=2)<>2 THEN RAISE EXCEPTION 'tasks not returned'; END IF;
 ...
 IF (SELECT count(*) FROM public.task_transition_history WHERE task_id IN ('16100000-0000-4000-8000-000000000001','16100000-0000-4000-8000-000000000002') AND command='return_to_team_for_member_suspension')<>2 THEN RAISE EXCEPTION 'history missing'; END IF;
 IF (SELECT count(*) FROM public.activity_events WHERE subject_id IN ('16100000-0000-4000-8000-000000000001','16100000-0000-4000-8000-000000000002') AND event_type='task.transitioned')<>2 THEN RAISE EXCEPTION 'activity missing'; END IF;
