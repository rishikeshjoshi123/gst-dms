...
DO $$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM public.tasks WHERE id='16100000-0000-4000-8000-000000000001' AND assignee_user_id IS NULL AND revision=3)
    OR NOT EXISTS(SELECT 1 FROM public.tasks WHERE id='16100000-0000-4000-8000-000000000002' AND assignee_user_id IS NULL AND revision=2) THEN RAISE EXCEPTION 'tasks not returned'; END IF;
 ...
 IF (SELECT count(*) FROM public.activity_events WHERE subject_id IN ('16100000-0000-4000-8000-000000000001','16100000-0000-4000-8000-000000000002') AND event_type='task.transitioned' AND metadata->>'command'='return_to_team_for_member_suspension')<>2 THEN RAISE EXCEPTION 'activity missing'; END IF;
