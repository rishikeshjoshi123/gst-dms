...
BEGIN
 ...
 IF has_function_privilege('authenticated','public.complete_document_upload(uuid,bigint,text,text,uuid,uuid,uuid)','EXECUTE') OR has_function_privilege('authenticated','public.cancel_document_upload(uuid,uuid,uuid,uuid)','EXECUTE') OR has_function_privilege('authenticated','public.maintain_document_upload_sessions(integer)','EXECUTE') THEN RAISE EXCEPTION 'service function grant'; END IF;
END $authenticated$;
RESET ROLE;
...
BEGIN
 ...
 IF (SELECT sr.reserved_bytes FROM public.storage_reservations AS sr WHERE sr.upload_session_id=sid)<>52428800 THEN RAISE EXCEPTION 'unobserved signed session must reserve Storage ceiling'; END IF;
 -- Give the remainder of this rollback-only fixture room for independent
 -- concurrency, cancellation, duplicate, and expiry sessions.
 UPDATE public.organisation_storage_policies SET unique_asset_entitlement_bytes=1073741824 WHERE org_id='36000000-0000-0000-0000-000000000001';
 UPDATE public.platform_storage_policy SET unique_asset_guard_bytes=2147483648 WHERE singleton;
...
DO $service$
DECLARE r record; first_session uuid; duplicate_session uuid; cross_session uuid; cancel_session uuid; count_before integer; retained_before bigint; retained_after bigint; h text:=repeat('a',64); denied boolean:=false;
BEGIN
 ...
 SELECT * INTO r FROM public.complete_document_upload(first_session,1024,h,'application/pdf','36500000-0000-0000-0000-000000000001','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'ok' THEN RAISE EXCEPTION 'unique completion'; END IF;
 SELECT * INTO r FROM public.complete_document_upload(first_session,1024,h,'application/pdf','36500000-0000-0000-0000-000000000001','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'ok' THEN RAISE EXCEPTION 'completion retry'; END IF;
 SELECT * INTO r FROM public.complete_document_upload(first_session,1024,h,'application/pdf','36500000-0000-0000-0000-000000000001','36100000-0000-0000-0000-000000000006','36000000-0000-0000-0000-000000000002'); IF r.code<>'not_found' THEN RAISE EXCEPTION 'cross identity completion receipt replay'; END IF;
 SELECT * INTO r FROM public.get_document_upload_completion_receipt(first_session,'36500000-0000-0000-0000-000000000099','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'not_found' THEN RAISE EXCEPTION 'wrong idempotency receipt replay'; END IF;
 SELECT * INTO r FROM public.cancel_document_upload(first_session,'36500000-0000-0000-0000-000000000001','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'already_completed' OR r.completion_code<>'ok' THEN RAISE EXCEPTION 'finalize must win before later cancel'; END IF;
 ...
 SELECT * INTO r FROM public.complete_document_upload(duplicate_session,5,h,'application/pdf','36500000-0000-0000-0000-000000000020','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'duplicate' OR r.duplicate_asset_id IS NULL THEN RAISE EXCEPTION 'same org duplicate'; END IF;
 ...
 SELECT * INTO r FROM public.complete_document_upload(cross_session,5,h,'application/pdf','36500000-0000-0000-0000-000000000021','36100000-0000-0000-0000-000000000006','36000000-0000-0000-0000-000000000002'); IF r.code<>'ok' THEN RAISE EXCEPTION 'cross org hash isolation'; END IF;
 ...
 -- Cancellation owns the session lock. Once it wins, completion cannot
 -- resurrect the upload, and a failed/unattempted Storage delete remains
 -- conservatively charged at the 50 MiB token ceiling.
 EXECUTE 'SET LOCAL ROLE authenticated'; SELECT * INTO r FROM public.reserve_document_upload('cancel.pdf','application/pdf',1,NULL,'36500000-0000-0000-0000-000000000040'); cancel_session:=r.upload_session_id; EXECUTE 'SET LOCAL ROLE service_role';
 SELECT * INTO r FROM public.cancel_document_upload(cancel_session,'36500000-0000-0000-0000-000000000040','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'cancelled' OR (SELECT state FROM public.upload_sessions WHERE id=cancel_session)<>'cancelled' OR (SELECT byte_size FROM public.file_assets WHERE id=r.asset_id)<>52428800 OR (SELECT storage_deleted_at FROM public.file_assets WHERE id=r.asset_id) IS NOT NULL THEN RAISE EXCEPTION 'cancel winner/accounting'; END IF;
 SELECT * INTO r FROM public.complete_document_upload(cancel_session,1,repeat('c',64),'application/pdf','36500000-0000-0000-0000-000000000040','36100000-0000-0000-0000-000000000001','36000000-0000-0000-0000-000000000001'); IF r.code<>'cancelled' THEN RAISE EXCEPTION 'cancelled upload finalized'; END IF;
 SELECT * INTO r FROM public.fail_document_upload(cancel_session,'validation_failed','36500000-0000-0000-0000-000000000040'); IF r.code<>'not_available' OR (SELECT state FROM public.upload_sessions WHERE id=cancel_session)<>'cancelled' OR EXISTS(SELECT 1 FROM public.document_upload_command_receipts WHERE upload_session_id=cancel_session AND command='fail') THEN RAISE EXCEPTION 'cancelled upload rewritten by failure'; END IF;
 IF EXISTS(SELECT 1 FROM public.claim_document_asset_storage_deletion_work(100) WHERE asset_id=r.asset_id) THEN RAISE EXCEPTION 'live token asset cleanup claim'; END IF;
 EXECUTE 'SET LOCAL ROLE authenticated'; SELECT * INTO r FROM public.reserve_document_upload('cancel.pdf','application/pdf',1,NULL,'36500000-0000-0000-0000-000000000040'); EXECUTE 'SET LOCAL ROLE service_role'; IF r.code<>'cancelled' THEN RAISE EXCEPTION 'cancelled upload token reissued'; END IF;
 ...
 SELECT * INTO r FROM public.maintain_document_upload_sessions(100); IF r.expired_sessions<1 OR (SELECT us.state FROM public.upload_sessions AS us WHERE us.id='36800000-0000-0000-0000-000000000001')<>'expired' OR (SELECT byte_size FROM public.file_assets WHERE id='36700000-0000-0000-0000-000000000001')<>52428800 OR (SELECT us.state FROM public.upload_sessions AS us WHERE us.idempotency_key='36500000-0000-0000-0000-000000000001')<>'finalized' THEN RAISE EXCEPTION 'expiry maintenance/accounting'; END IF;
 ...
 IF retained_after<>retained_before-52428800
