...
test('the shared PDF viewer owns one responsive scroller and fit-width toolbar', async () => {
  ...
  assert.match(source, /min-h-0 w-full flex-1 flex-col items-center gap-4 overflow-auto/)
...
test('source and page failures are distinct, non-disclosing, and recoverable', async () => {
  const source = await readFile(new URL('./pdf-viewer-client.tsx', import.meta.url), 'utf8')

  assert.match(source, /onLoadError=\{onDocumentLoadError\}/)
  assert.match(source, /onSourceError=\{onDocumentSourceError\}/)
  assert.match(source, /onPassword=\{\(\) => presentSourceFailure\('encrypted'\)\}/)
  assert.match(source, /retryPdfSourceAccess\(\{/)
  assert.match(source, /requestFreshUrl: onRequestSourceRefresh/)
  assert.match(source, /refreshRoute: router\.refresh/)
  assert.match(source, /sourceFailureCopy\.retryLabel/)
  assert.match(source, /Page \{page\} could not be rendered/)
