...
function DeadlineEditor({ matterId, item, mode, open, onOpenChange, returnFocus }: { matterId: string; item?: ManualDeadlineItem; mode: EditorMode; open: boolean; onOpenChange(open: boolean): void; returnFocus: React.RefObject<HTMLButtonElement | null> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent onCloseAutoFocus={(event)=>{event.preventDefault();returnFocus.current?.focus()}} className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-xl">
    <DialogHeader><DialogTitle>{mode === 'create' ? 'Add legal deadline' : 'Amend legal deadline'}</DialogTitle><DialogDescription>{mode === 'create' ? 'A manual date begins verified. State the source or legal basis explicitly.' : 'The current version remains in history. Explain why this amendment is needed.'}</DialogDescription></DialogHeader>
    <form onSubmit={submit} className="space-y-4">
      ...
      <DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Keep unchanged</Button><Button type="submit" loading={busy}>{mode === 'create' ? 'Add deadline' : 'Amend deadline'}</Button></DialogFooter>
    </form>
...
function DeadlineBadges({ item }: { item:ManualDeadlineItem }) { return <div className="flex flex-wrap gap-1.5"><Badge variant={temporalVariant[item.temporal]} fixedWidth="lg">{TEMPORAL_LABELS[item.temporal]}</Badge><Badge variant={item.lifecycle==='open'?'default':item.lifecycle==='satisfied'?'success':'muted'} fixedWidth="lg">{item.lifecycle==='open'?'Open':item.lifecycle==='satisfied'?'Satisfied':'Cancelled'}</Badge></div> }
...
export function MatterDeadlineAgenda({ matterId, agenda, readOnly=false }: { matterId:string; agenda:ManualDeadlineAgenda; readOnly?:boolean }) {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden pt-2 md:pt-3">
    <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-[var(--border-subtle)] bg-[var(--surface)] px-3 py-2 md:px-4"><div className="min-w-0 flex-1"><p className="text-sm font-medium">Verified manual agenda</p><p className="truncate text-xs text-[var(--text-muted)]">Date-only status uses {agenda.timezone} · as of {dateLabel(agenda.asOfDate)}</p></div>{canMutate&&<Button size="sm" onClick={(event)=>{editorReturnFocus.current=event.currentTarget;setEditor('create')}}><Plus className="size-4"/>Add deadline</Button>}{!canMutate&&<Badge variant="muted">Read only</Badge>}</div>
    {agenda.items.length===0?<div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-6"><div className="max-w-sm text-center"><CalendarClock className="mx-auto size-7 text-[var(--text-muted)]"/><h2 className="mt-3 text-section-heading">No verified manual deadlines</h2><p className="mt-1 text-sm text-[var(--text-muted)]">Add a human-checked legal obligation and date. Extracted candidates and reminders are not shown in this release.</p></div></div>:
      <div className="grid min-h-0 flex-1 overflow-y-auto overscroll-contain xl:grid-cols-[minmax(0,3fr)_minmax(20rem,2fr)] xl:overflow-hidden">
        ...
        {selected&&<aside aria-label="Selected deadline details" className={cn('min-h-0 overflow-visible border-t border-[var(--border)] bg-[var(--surface)] p-4 xl:overflow-y-auto xl:overscroll-contain xl:border-l xl:border-t-0',!mobileDetail&&'hidden md:block')}>
          ...
          <div className="mt-4 flex flex-wrap gap-1.5"><Badge variant="outline">Legal deadline</Badge><Badge variant="outline">Manual origin</Badge><Badge variant="success">Verified</Badge></div><div className="mt-3"><DeadlineBadges item={selected}/></div>
          ...
          {canMutate&&selected.lifecycle==='open'&&<div className="mt-5 flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={(event)=>{editorReturnFocus.current=event.currentTarget;setEditor('amend')}}><Pencil className="size-4"/>Amend deadline</Button><Button variant="secondary" size="sm" onClick={()=>setOutcome('satisfied')}><CheckCircle2 className="size-4"/>Mark satisfied</Button><Button variant="destructive" size="sm" onClick={()=>setOutcome('cancelled')}><XCircle className="size-4"/>Cancel deadline</Button></div>}
          <section className="mt-6"><h3 className="text-sm font-semibold">History</h3><ol className="mt-3 space-y-3 border-l border-[var(--border-strong)] pl-4">{selected.history.map(entry=><li key={`${entry.kind}-${entry.revision}`} className="text-sm"><p className="font-medium">{entry.kind==='created'?'Deadline created':entry.kind==='amended'?'Deadline amended':entry.kind==='satisfied'?'Marked satisfied':'Deadline cancelled'}</p><p className="mt-0.5 text-xs text-[var(--text-muted)]">Revision {entry.revision} · {entry.actor_label} · {new Intl.DateTimeFormat('en-IN',{dateStyle:'medium',timeStyle:'short'}).format(new Date(entry.at))}</p>{entry.due_date&&<p className="mt-1 break-words text-xs">{dateLabel(entry.due_date)} · {entry.manual_basis}</p>}{entry.reason&&<p className="mt-1 break-words text-xs text-[var(--text-secondary)]">Reason: {entry.reason}</p>}</li>)}</ol></section>
        </aside>}
      </div>}
    <DeadlineEditor
      matterId={matterId}
