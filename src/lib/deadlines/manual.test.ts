...
test('authority source retains fixed search paths, explicit role mutation policy, actor-bound replay, CAS, direct-write closure and no reminder claim',()=>{
  ...
  assert.match(migration,/DROP POLICY IF EXISTS deadlines_select/)
  assert.match(migration,/REVOKE SELECT,INSERT,UPDATE,DELETE ON public\.deadlines FROM PUBLIC,authenticated,anon/)
  assert.match(migration,/description=normalized_legacy\.obligation[\s\S]*title=left\(normalized_legacy\.obligation,160\)/)
  assert.match(migration,/CREATE FUNCTION public\.read_current_deadline_attention[\s\S]*SECURITY DEFINER SET search_path=pg_catalog,public/)
  assert.match(notifications,/rpc\('read_current_deadline_attention'/)
  assert.doesNotMatch(notifications,/\.from\('deadlines'\)/)
  ...
  assert.match(ui,/Legal deadline/); assert.match(ui,/item\.origin==='manual'/); assert.match(ui,/Verified/)
})
