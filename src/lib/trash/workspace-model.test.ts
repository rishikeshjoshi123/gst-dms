...
test('server and live UI sources preserve the root-only Restore boundary', () => {
  ...
  const purgeVisibilityMigration = readFileSync(new URL('../../../supabase/migrations/00092_trash_workspace_purge_status_visibility.sql', import.meta.url), 'utf8')
  ...
  assert.match(ui, /Review permanent deletion/)
  assert.match(ui, /operation\.purgeImpact\?\.canPurge === true/)
  assert.match(ui, /View permanent deletion status/)
  assert.match(reader, /get_trash_purge_impact/)
  assert.match(ui, /\/permanent-delete/)
  ...
  assert.match(purgeVisibilityMigration, /'purging'::public\.trash_operation_state/)
  assert.match(purgeVisibilityMigration, /'purge_failed'::public\.trash_operation_state/)
  assert.doesNotMatch(purgeVisibilityMigration, /object_key|storage_path|raw_metadata/)
})
