...
import { continueProcessingManually, previewMultiPlacementConflictMove, previewPlacementConflictMove, resolveAmbiguousPlacement, resolveExplicitDueDate, resolveExtractionConflict, resolveMultiPlacementConflict, resolvePlacementConflict, resolvePossibleDuplicate, resolveRelationshipSuggestion } from '@/lib/actions/review'
import { reviewFieldLabel, reviewValueLabel, taxPeriodComparison, type AmbiguousPlacementResolution, type DeadlineReviewResolution, type ExtractionReviewResolution, type MultiPlacementConflictResolution, type PlacementConflictResolution, type PossibleDuplicateResolution, type ProcessingRecoveryResolution, type RelationshipSuggestionResolution, type ReviewDetail, type ReviewFilters, type ReviewQueueItem } from '@/lib/review/model'
...
const typeLabel = (type: ReviewQueueItem['type']) => type === 'relationship_suggestion' ? 'Reference relationship' : type === 'possible_duplicate' ? 'Possible duplicate' : type === 'processing_recovery' ? 'Processing recovery' : type === 'ambiguous_placement' ? 'Intake placement conflict' : type === 'placement_conflict' ? 'Filed document placement conflict' : type === 'multi_placement_conflict' ? 'Multiple filed Matter conflicts' : type === 'deadline_verification' ? 'Legal date verification' : 'Extraction conflict'
const itemLabel = (item: ReviewQueueItem | ReviewDetail) => item.type === 'relationship_suggestion' ? 'Review exact reference relationship' : item.type === 'possible_duplicate' ? `Compare ${item.source_count??2} documents` : item.type === 'processing_recovery' ? 'Continue document manually' : item.type === 'ambiguous_placement' ? 'Choose a Matter destination' : item.type === 'placement_conflict' || item.type === 'multi_placement_conflict' ? 'Review current filing' : item.type === 'deadline_verification' ? 'Review proposed legal date' : `Review ${reviewFieldLabel(item.field_path)}`
type DuplicateValue={document_id:string;document_version_id:string;document_title:string;version_number:number;matter_title:string;client_name:string;display:string;kind:string;namespace:string;normalized_value:string;verified_at:string}
function duplicateValue(value:unknown):DuplicateValue|null{
  if(!value||typeof value!=='object'||Array.isArray(value)) return null
  const fact=value as Record<string,unknown>
  return typeof fact.document_id==='string'&&typeof fact.document_version_id==='string'&&typeof fact.document_title==='string' ? fact as DuplicateValue : null
}
type RelationshipValue={source_document_id:string;source_document_version_id:string;source_document_title:string;target_document_id:string;target_document_version_id:string;target_document_title:string;identifier_kind:string;namespace:string;normalized_value:string}
function relationshipValue(value:unknown):RelationshipValue|null{
  if(!value||typeof value!=='object'||Array.isArray(value)) return null
  const fact=value as Record<string,unknown>
  return typeof fact.source_document_id==='string'&&typeof fact.target_document_id==='string'&&typeof fact.source_document_title==='string'&&typeof fact.target_document_title==='string' ? fact as RelationshipValue : null
}
...
export function ReviewClientView({ queue, detail, filters, asOf }: { queue: Queue; detail: ReviewDetail | null; filters: ReviewFilters; asOf: number }) {
  ...
  const filterControls = <>
    <Filter label="Type" value={filters.type} options={[["all", "All"], ["extraction_conflict", "Extraction conflict"], ["processing_recovery", "Processing recovery"], ["ambiguous_placement", "Intake placement conflict"], ["placement_conflict", "Filed placement conflict"], ["multi_placement_conflict", "Multiple filed conflicts"], ["deadline_verification", "Legal date verification"], ["possible_duplicate", "Possible duplicate"], ["relationship_suggestion", "Reference relationship"]]} onChange={type => navigate({ type, page: undefined })} />
...
function ReviewItemDetail({ detail, tab, onDecisionTab, asOf }: { detail: ReviewDetail; tab: ReviewFilters['tab']; onDecisionTab: () => void; asOf: number }) {
  ...
  const [selectedDuplicateDocumentIds,setSelectedDuplicateDocumentIds]=useState<string[]>([])
  ...
  const [confirmation, setConfirmation] = useState<ExtractionReviewResolution | ProcessingRecoveryResolution | AmbiguousPlacementResolution | DeadlineReviewResolution | PlacementConflictResolution | MultiPlacementConflictResolution | PossibleDuplicateResolution | RelationshipSuggestionResolution | null>(null)
  ...
  const router = useRouter()
  const params=useSearchParams()
  const [relationshipType,setRelationshipType]=useState(itemRelationshipType(detail))
  const [relationshipReversed,setRelationshipReversed]=useState(false)
  ...
  const duplicateDocuments=item.type==='possible_duplicate' ? item.evidence.map(evidence=>duplicateValue(evidence.value)).filter((value):value is DuplicateValue=>value!==null) : []
  const selectedDuplicateDocuments=duplicateDocuments.filter(value=>selectedDuplicateDocumentIds.includes(value.document_id))
  const relationshipEvidence=item.type==='relationship_suggestion'?relationshipValue(item.evidence[0]?.value):null
  const selectedRelationshipCatalogue=item.type==='relationship_suggestion'?item.relationship_catalogue?.find(option=>option.relationship_type===relationshipType):undefined
  ...
  function confirm() {
    ...
    startTransition(async () => {
      try {
        if(confirmation.action==='accept_relationship'||confirmation.action==='correct_relationship'||confirmation.action==='reject_relationship'){
          const result=await resolveRelationshipSuggestion(confirmation)
          setMessage(result.message)
          if(result.item)setItem(result.item)
          if(result.code!=='failed'&&result.code!=='cycle_detected'&&result.code!=='invalid_relationship_type'){setConfirmation(null);setChoice('');setReason('');router.refresh()}
          return
        }
        if(confirmation.action==='distinct_documents'||confirmation.action==='possible_same_document'){
          const result=await resolvePossibleDuplicate(confirmation)
          setMessage(result.message)
          if(result.item)setItem(result.item)
          if(result.code!=='failed'){setConfirmation(null);setChoice('');setSelectedDuplicateDocumentIds([]);setReason('');router.refresh()}
          return
        }
  ...
  return <>
    ...
    <div key={tab} id="review-tabpanel" role="tabpanel" aria-labelledby={`review-tab-${tab}`} tabIndex={0} className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4">
      ...
      {tab === 'evidence' ? <>
        ...
        {item.type==='possible_duplicate' && <section className="space-y-3 border-t border-[var(--border)] pt-3">
          <h3 className="text-sm font-medium">Exact official key in {item.evidence.length} current PDFs</h3>
          <p className="text-sm">These are separate logical documents and separate private PDF assets. An exact shared source-grounded identifier is a warning, not proof that the documents are identical. Neither source or filing changes automatically.</p>
          <div className="grid min-w-0 gap-3 2xl:grid-cols-2">{item.evidence.map(candidate=>{const value=duplicateValue(candidate.value);return value&&<article key={candidate.candidate_id} className="min-w-0 space-y-2 rounded-[var(--radius-md)] border border-[var(--border)] p-3">
            <h4 className="break-words text-sm font-medium">{value.document_title} · Version {value.version_number}</h4>
            <p className="break-words text-caption text-[var(--text-muted)]">{value.client_name} · {value.matter_title}</p>
            ...
            <p className="break-words font-mono text-caption">{value.kind.replaceAll('_',' ')} · {value.namespace} · {value.normalized_value}</p>
            <p className="break-words text-caption text-[var(--text-muted)]">Source display: {value.display} · verified {new Date(value.verified_at).toLocaleDateString()}</p>
            <Link className="inline-flex min-h-11 items-center text-sm text-[var(--primary)] underline underline-offset-4" href={canonicalDocumentPath(value.document_id,{version:value.document_version_id,page:String(candidate.page_number)})}>Open exact PDF source · Page {candidate.page_number}</Link>
            <Link className="inline-flex min-h-11 items-center text-sm text-[var(--primary)] underline underline-offset-4" href={canonicalDocumentPath(value.document_id,{version:value.document_version_id})}>Inspect version in Workbench</Link>
          </article>})}</div>
        </section>}
        {item.type==='relationship_suggestion' && relationshipEvidence && <section className="space-y-3 border-t border-[var(--border)] pt-3">
          <h3 className="text-sm font-medium">Exact reference in the current PDF</h3>
          <p className="text-sm">This source resolves exactly to one current proceeding in the same Matter. It proposes only <span className="font-mono">refers_to</span>; no relationship type is inferred from the wording.</p>
          ...
          <p className="break-words font-mono text-caption">{relationshipEvidence.identifier_kind.replaceAll('_',' ')} · {relationshipEvidence.namespace} · {relationshipEvidence.normalized_value}</p>
          <div className="flex flex-wrap gap-3"><Link className="inline-flex min-h-11 items-center text-sm text-[var(--primary)] underline underline-offset-4" href={canonicalDocumentPath(relationshipEvidence.source_document_id,{version:relationshipEvidence.source_document_version_id,page:String(item.evidence[0].page_number)})}>Open exact PDF source · Page {item.evidence[0].page_number}</Link><Link className="inline-flex min-h-11 items-center text-sm text-[var(--primary)] underline underline-offset-4" href={canonicalDocumentPath(relationshipEvidence.target_document_id,{version:relationshipEvidence.target_document_version_id})}>Inspect referenced document</Link></div>
          <dl className="grid gap-2 text-sm"><div><dt className="text-caption text-[var(--text-muted)]">Canonical proposal</dt><dd>{relationshipEvidence.source_document_title} refers to {relationshipEvidence.target_document_title}</dd></div><div><dt className="text-caption text-[var(--text-muted)]">Timeline effect</dt><dd>Accepted <span className="font-mono">refers_to</span> remains inspectable, but is not a Timeline progression edge.</dd></div></dl>
          {item.current_matter_id&&<Link className="inline-flex min-h-11 items-center rounded-[var(--radius-sm)] border border-[var(--border-strong)] px-3 text-sm font-medium" href={`/matters/${encodeURIComponent(item.current_matter_id)}?section=timeline&view=graph&reviewItem=${encodeURIComponent(item.id)}&reviewRevision=${item.revision}&from=review&returnTo=${encodeURIComponent(`/review?${params.toString()}`)}`}>Review on timeline</Link>}
        </section>}
        ...
        {item.type==='possible_duplicate'&&item.last_decision&&<section className="space-y-2 border-t border-[var(--border)] pt-3"><h3 className="text-sm font-medium">{item.last_decision.action==='distinct_documents'?'Distinct documents recorded':'Possible same document recorded'}</h3><p className="break-words text-sm">{item.last_decision.action==='distinct_documents'?`This comparison is closed: all ${item.evidence.length} cited PDFs are recorded as distinct. The unchanged full source set will not raise Review again.`:'This comparison is closed as a finding only for the selected documents below. No relationship to an unselected member is inferred. Every cited PDF version and logical document remains separate and unchanged; any remediation requires a separate governed Workbench action.'}</p>{item.last_decision.action==='possible_same_document'&&<div><p className="text-caption font-medium">Selected possible-same documents</p><ul className="mt-1 list-disc space-y-1 pl-5 text-sm">{duplicateDocuments.filter(value=>item.last_decision?.selected_document_ids?.includes(value.document_id)).map(value=><li key={value.document_id} className="break-words">{value.document_title} · Version {value.version_number} · {value.document_id}</li>)}</ul></div>}<p className="break-words text-sm">Reason: {item.last_decision.reason}</p></section>}
        {item.type==='relationship_suggestion'&&item.last_decision&&<section className="space-y-2 border-t border-[var(--border)] pt-3"><h3 className="text-sm font-medium">{item.last_decision.action==='reject_relationship'?'Relationship suggestion rejected':'Relationship decision recorded'}</h3><p className="text-sm">{item.last_decision.action==='reject_relationship'?'No relationship was created.':item.last_decision.action==='accept_relationship'?'The exact source reference was accepted as an inspectable refers_to relationship. It is not a Timeline progression edge.':'The relationship type or direction was human-corrected before activation.'}</p><p className="break-words text-sm">Reason: {item.last_decision.reason}</p></section>}
        {item.last_decision && item.type!=='possible_duplicate' && item.type!=='relationship_suggestion' && <section className="space-y-2"><h3 className="text-sm font-medium">{item.type==='deadline_verification'?'Legal date decision recorded':(item.type==='placement_conflict'||item.type==='multi_placement_conflict') && item.last_decision.action==='keep' ? 'Current filing kept' : (item.type==='placement_conflict'||item.type==='multi_placement_conflict') && item.last_decision.action==='move' ? 'Document moved' : item.last_decision.action === 'select_candidate' ? 'Recorded outcome' : item.last_decision.action === 'continue_manual' ? 'Manual continuation recorded' : item.last_decision.action === 'select_destination' ? 'Placement recorded' : 'Clarification requested'}</h3>{item.type!=='deadline_verification'&&item.type!=='multi_placement_conflict'&&item.last_decision.selected_candidate_id && <p className="break-words text-sm">Selected: {reviewValueLabel(item.evidence.find(e => e.candidate_id === item.last_decision?.selected_candidate_id)?.value)}</p>}{item.last_decision.placement_candidate_id && <p className="break-words text-sm">Destination: {placementValue(item.evidence.find(e => e.candidate_id === item.last_decision?.placement_candidate_id)?.value)?.matter_title}</p>}{item.type==='placement_conflict' && <p className="break-words text-sm">Recorded {item.last_decision.action==='move' ? `Move from ${recordedSource} to ${recordedTarget}` : `Keep in ${recordedSource}`}.</p>}{item.type==='multi_placement_conflict' && <p className="break-words text-sm">Recorded {item.last_decision.action==='move' ? `Move from ${recordedSource} to ${multiCandidateValue(item.evidence.find(e=>e.candidate_id===item.last_decision?.selected_candidate_id)?.value)?.target_matter_title??'the chosen Matter'}` : `Keep in ${recordedSource}`}.</p>}<p className="break-words text-sm">{item.type==='placement_conflict'||item.type==='multi_placement_conflict' ? `Reason: ${item.last_decision.reason}` : item.last_decision.reason}</p></section>}
        {canResolve && item.type==='relationship_suggestion'&&relationshipEvidence ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide this exact reference</legend>
          <p className="text-caption text-[var(--text-muted)]">Accept records only the conservative source → target <span className="font-mono">refers_to</span>. Correct lets you choose the direction and one current catalogue type. Reject creates no relationship.</p>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="relationship-outcome" checked={choice==='accept_relationship'} onChange={()=>{setChoice('accept_relationship');setRelationshipType(item.suggested_relationship_type??'refers_to');setRelationshipReversed(false)}} className="mt-1 size-4 accent-[var(--primary)]"/><span className="text-sm">Accept source → target refers to<span className="mt-1 block text-caption text-[var(--text-muted)]">Inspectable relationship only; not a Timeline progression edge.</span></span></label>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="relationship-outcome" checked={choice==='correct_relationship'} onChange={()=>setChoice('correct_relationship')} className="mt-1 size-4 accent-[var(--primary)]"/><span className="text-sm">Correct relationship type or direction</span></label>
          {choice==='correct_relationship'&&<div className="space-y-3 border-l-2 border-[var(--border-strong)] pl-3"><div><Label htmlFor="relationship-type">Current relationship type</Label><select id="relationship-type" className="input-base min-h-11" value={relationshipType} onChange={event=>setRelationshipType(event.target.value as RelationshipSuggestionResolution['relationshipType']&string)}>{item.relationship_catalogue?.map(option=><option key={option.relationship_type} value={option.relationship_type}>{option.canonical_phrase}{option.timeline_visible?' · Timeline progression':' · Inspectable only'}</option>)}</select></div><label className="flex min-h-11 cursor-pointer items-center gap-3"><input type="checkbox" checked={relationshipReversed} onChange={event=>setRelationshipReversed(event.target.checked)} className="size-4 accent-[var(--primary)]"/><span className="text-sm">Reverse canonical direction to {relationshipEvidence.target_document_title} → {relationshipEvidence.source_document_title}</span></label><p className="text-caption text-[var(--text-muted)]">{selectedRelationshipCatalogue?.timeline_visible?'This type can appear as a Timeline progression edge after acceptance.':'This type remains inspectable and does not appear as a Timeline progression edge.'}</p></div>}
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="relationship-outcome" checked={choice==='reject_relationship'} onChange={()=>setChoice('reject_relationship')} className="mt-1 size-4 accent-[var(--primary)]"/><span className="text-sm">Reject suggestion<span className="mt-1 block text-caption text-[var(--text-muted)]">Record the reason and create no relationship.</span></span></label>
        </fieldset> : canResolve && item.type==='possible_duplicate' ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide what these {item.evidence.length} documents represent</legend>
          <p className="text-caption text-[var(--text-muted)]">Compare every exact Workbench source first. This decision records your interpretation only. It cannot merge, remove, replace, move, or change effective facts.</p>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="duplicate-outcome" className="mt-1 size-4 accent-[var(--primary)]" checked={choice==='distinct_documents'} onChange={()=>{setChoice('distinct_documents');setSelectedDuplicateDocumentIds([])}}/><span className="text-sm">Mark all as distinct documents<span className="mt-1 block text-caption text-[var(--text-muted)]">All {item.evidence.length} cited sources are distinct. The unchanged full source set will not be raised again.</span></span></label>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="duplicate-outcome" className="mt-1 size-4 accent-[var(--primary)]" checked={choice==='possible_same_document'} onChange={()=>{setChoice('possible_same_document');setSelectedDuplicateDocumentIds(duplicateDocuments.length===2?duplicateDocuments.map(value=>value.document_id):[])}}/><span className="text-sm">Mark a selected subset as possibly the same document<span className="mt-1 block text-caption text-[var(--text-muted)]">Name at least two current documents. No relation to an unselected PDF is inferred; all records stay intact.</span></span></label>
          {choice==='possible_same_document'&&<div className="space-y-2 border-l-2 border-[var(--border-strong)] pl-3"><p className="text-caption">Select the exact documents in this possible-same finding ({selectedDuplicateDocumentIds.length} selected).</p>{duplicateDocuments.map(value=><label key={value.document_id} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-2"><input type="checkbox" checked={selectedDuplicateDocumentIds.includes(value.document_id)} onChange={event=>setSelectedDuplicateDocumentIds(current=>event.target.checked?[...current,value.document_id]:current.filter(id=>id!==value.document_id))} className="size-4 accent-[var(--primary)]"/><span className="min-w-0 break-words text-sm">{value.document_title} · Version {value.version_number} · {value.document_id}</span></label>)}</div>}
        </fieldset> : canResolve && item.type==='multi_placement_conflict' ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide this current filing</legend>
        ...
        </fieldset> : canResolve && item.type==='extraction_conflict' ? <fieldset className="space-y-3"><legend className="mb-3 text-sm font-medium">Choose an outcome</legend>
    ...
    <footer className="shrink-0 border-t border-[var(--border)] p-3">
      {tab === 'evidence' ? <Button variant="outline" className="w-full" onClick={onDecisionTab}>View decision</Button> : canResolve && item.type==='relationship_suggestion'&&relationshipEvidence ? <Button className="w-full" disabled={!choice||pending||(choice==='correct_relationship'&&!selectedRelationshipCatalogue)} onClick={()=>{const reject=choice==='reject_relationship';const reverse=choice==='correct_relationship'&&relationshipReversed;setConfirmation({itemId:item.id,revision:item.revision,action:choice as RelationshipSuggestionResolution['action'],relationshipType:reject?null:choice==='accept_relationship'?'refers_to':relationshipType,catalogueVersion:reject?null:choice==='accept_relationship'?item.suggested_catalogue_version??null:selectedRelationshipCatalogue?.catalogue_version??null,sourceDocumentId:reject?null:reverse?relationshipEvidence.target_document_id:relationshipEvidence.source_document_id,targetDocumentId:reject?null:reverse?relationshipEvidence.source_document_id:relationshipEvidence.target_document_id,reason:'',idempotencyKey:crypto.randomUUID()})}}>Review relationship decision</Button> : canResolve && item.type==='possible_duplicate' ? <Button className="w-full" disabled={!choice||pending||(choice==='possible_same_document'&&selectedDuplicateDocumentIds.length<2)} onClick={()=>setConfirmation({itemId:item.id,revision:item.revision,action:choice as PossibleDuplicateResolution['action'],selectedDocumentIds:choice==='distinct_documents'?[]:selectedDuplicateDocumentIds,reason:'',idempotencyKey:crypto.randomUUID()})}>Review document interpretation</Button> : canResolve && item.type==='multi_placement_conflict' ? <Button className="w-full" disabled={!choice || previewPending || pending || (choice!=='keep' && !hasCurrentMultiMoveImpact(multiMovePreview,item.id,choice,previewPending))} onClick={()=>{
    ...
    <Dialog open={confirmation !== null} onOpenChange={open => { if (!open && !pending) { setConfirmation(null); setReason('') } }}><DialogContent><DialogHeader><DialogTitle>{confirmation?.action==='accept_relationship'?'Accept this reference relationship?':confirmation?.action==='correct_relationship'?'Record the corrected relationship?':confirmation?.action==='reject_relationship'?'Reject this relationship suggestion?':confirmation?.action==='distinct_documents'?'Record these as distinct documents?':confirmation?.action==='possible_same_document'?'Record the selected documents as possibly the same?':confirmation?.action==='keep'?'Keep this document in its current Matter?':confirmation?.action==='move'?'Move this document to the chosen Matter?':confirmation && ['verify','correct','reject','clear'].includes(confirmation.action) ? 'Record this legal date decision?' : confirmation?.action === 'select_candidate' ? 'Use this extracted value?' : confirmation?.action === 'continue_manual' ? 'Continue with manual metadata?' : confirmation?.action === 'select_destination' ? 'Place this Intake PDF?' : 'Request clarification?'}</DialogTitle><DialogDescription>{confirmation?.action==='accept_relationship'?`Record ${relationshipEvidence?.source_document_title} refers to ${relationshipEvidence?.target_document_title}. This remains inspectable and is not a Timeline progression edge.`:confirmation?.action==='correct_relationship'?`Record the human-corrected ${selectedRelationshipCatalogue?.canonical_phrase??relationshipType} relationship in the chosen direction. ${selectedRelationshipCatalogue?.timeline_visible?'It may appear as a Timeline progression edge.':'It remains inspectable and is not a Timeline progression edge.'}`:confirmation?.action==='reject_relationship'?'Close this exact-source suggestion without creating any relationship.':confirmation?.action==='distinct_documents'?`Record all ${item.evidence.length} cited current PDF versions as distinct. The unchanged full source set will not raise Review again; every document, source, filing and effective fact remains untouched.`:confirmation?.action==='possible_same_document'?`Record a possible-same finding for only these ${selectedDuplicateDocuments.length} selected documents: ${selectedDuplicateDocuments.map(value=>`${value.document_title} (${value.document_id})`).join('; ')}. No relation to any unselected source is inferred. No merge, deletion, move, replacement or effective-fact change will occur.`:confirmation?.action==='keep'?`Keep ${item.document_title} in ${item.matter_title}. The verified candidate set and your reason remain in Review history; unchanged source evidence will not raise this conflict again.`:confirmation?.action==='move'?`Move ${item.document_title} from ${item.matter_title} to ${item.type==='multi_placement_conflict'?multiCandidateValue(selected?.value)?.target_matter_title:item.target_matter_title}. This target’s current impact will be rechecked transactionally; the same document, version and PDF source remain available.`:confirmation && ['verify','correct','reject','clear'].includes(confirmation.action) ? `${confirmation.action==='correct'?'Correct to '+correctedDate:confirmation.action==='verify'?'Verify the cited date '+item.candidate_due_date:confirmation.action==='reject'?'Reject this candidate':'Clear this proposed date'}. The source stays immutable; your reason and decision enter history.` : confirmation?.action === 'select_candidate' ? `Use ${reviewValueLabel(selected?.value)}. Competing item candidates will be rejected and this Review item will close.` : confirmation?.action === 'continue_manual' ? 'Record the verified document details, close this recovery item, and continue in the existing Workbench. Automated extraction will not be retried.' : confirmation?.action === 'select_destination' ? `Create the canonical document in ${placementValue(selected?.value)?.matter_title ?? 'the selected Matter'}, close Review, and open the exact new version at page 1.` : 'Record what needs clarification. The current value stays unchanged and this item remains in Needs review.'}</DialogDescription></DialogHeader>
      ...
      <DialogFooter><Button variant="ghost" disabled={pending} onClick={() => { setConfirmation(null); setReason('') }}>Return to decision</Button><Button loading={pending} disabled={reason.trim().length<2} onClick={confirm}>{confirmation?.action==='accept_relationship'?'Accept relationship':confirmation?.action==='correct_relationship'?'Record correction':confirmation?.action==='reject_relationship'?'Reject suggestion':confirmation?.action==='distinct_documents'?'Record distinct documents':confirmation?.action==='possible_same_document'?'Record possible same document':confirmation?.action==='keep'?'Keep current filing':confirmation?.action==='move'?'Move document':confirmation && ['verify','correct','reject','clear'].includes(confirmation.action) ? 'Record date decision' : confirmation?.action === 'select_candidate' ? 'Confirm selected value' : confirmation?.action === 'continue_manual' ? 'Continue manually' : confirmation?.action === 'select_destination' ? 'Place in selected Matter' : 'Request clarification'}</Button></DialogFooter>
    </DialogContent></Dialog>
  </>
}

function itemRelationshipType(item:ReviewDetail):RelationshipSuggestionResolution['relationshipType']&string{
  return item.type==='relationship_suggestion'?(item.suggested_relationship_type??item.relationship_catalogue?.[0]?.relationship_type??'refers_to'):'refers_to'
}
