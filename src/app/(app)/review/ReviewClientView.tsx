...
import { continueProcessingManually, previewPlacementConflictMove, resolveAmbiguousPlacement, resolveExplicitDueDate, resolveExtractionConflict, resolvePlacementConflict } from '@/lib/actions/review'
import { reviewFieldLabel, reviewValueLabel, taxPeriodComparison, type AmbiguousPlacementResolution, type DeadlineReviewResolution, type ExtractionReviewResolution, type PlacementConflictResolution, type ProcessingRecoveryResolution, type ReviewDetail, type ReviewFilters, type ReviewQueueItem } from '@/lib/review/model'
...
const typeLabel = (type: ReviewQueueItem['type']) => type === 'processing_recovery' ? 'Processing recovery' : type === 'ambiguous_placement' ? 'Intake placement conflict' : type === 'placement_conflict' ? 'Filed document placement conflict' : type === 'deadline_verification' ? 'Legal date verification' : 'Extraction conflict'
const itemLabel = (item: ReviewQueueItem | ReviewDetail) => item.type === 'processing_recovery' ? 'Continue document manually' : item.type === 'ambiguous_placement' ? 'Choose a Matter destination' : item.type === 'placement_conflict' ? 'Review current filing' : item.type === 'deadline_verification' ? 'Review proposed legal date' : `Review ${reviewFieldLabel(item.field_path)}`
...
export function ReviewClientView({ queue, detail, filters, asOf }: { queue: Queue; detail: ReviewDetail | null; filters: ReviewFilters; asOf: number }) {
  ...
  const filterControls = <>
    <Filter label="Type" value={filters.type} options={[["all", "All"], ["extraction_conflict", "Extraction conflict"], ["processing_recovery", "Processing recovery"], ["ambiguous_placement", "Intake placement conflict"], ["placement_conflict", "Filed placement conflict"], ["deadline_verification", "Legal date verification"]]} onChange={type => navigate({ type, page: undefined })} />
...
function ReviewItemDetail({ detail, tab, onDecisionTab, asOf }: { detail: ReviewDetail; tab: ReviewFilters['tab']; onDecisionTab: () => void; asOf: number }) {
  ...
  const [confirmation, setConfirmation] = useState<ExtractionReviewResolution | ProcessingRecoveryResolution | AmbiguousPlacementResolution | DeadlineReviewResolution | PlacementConflictResolution | null>(null)
    ...
    startTransition(async () => {
      try {
        if (confirmation.action==='keep' || confirmation.action==='move'){
          const result=await resolvePlacementConflict(confirmation)
