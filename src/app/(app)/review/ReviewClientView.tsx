...
import { continueProcessingManually, previewMultiPlacementConflictMove, previewPlacementConflictMove, resolveAmbiguousPlacement, resolveExplicitDueDate, resolveExtractionConflict, resolveMultiPlacementConflict, resolvePlacementConflict, resolvePossibleDuplicate } from '@/lib/actions/review'
import { reviewFieldLabel, reviewValueLabel, taxPeriodComparison, type AmbiguousPlacementResolution, type DeadlineReviewResolution, type ExtractionReviewResolution, type MultiPlacementConflictResolution, type PlacementConflictResolution, type PossibleDuplicateResolution, type ProcessingRecoveryResolution, type ReviewDetail, type ReviewFilters, type ReviewQueueItem } from '@/lib/review/model'
...
const typeLabel = (type: ReviewQueueItem['type']) => type === 'possible_duplicate' ? 'Possible duplicate' : type === 'processing_recovery' ? 'Processing recovery' : type === 'ambiguous_placement' ? 'Intake placement conflict' : type === 'placement_conflict' ? 'Filed document placement conflict' : type === 'multi_placement_conflict' ? 'Multiple filed Matter conflicts' : type === 'deadline_verification' ? 'Legal date verification' : 'Extraction conflict'
const itemLabel = (item: ReviewQueueItem | ReviewDetail) => item.type === 'possible_duplicate' ? `Compare ${item.source_count??2} documents` : item.type === 'processing_recovery' ? 'Continue document manually' : item.type === 'ambiguous_placement' ? 'Choose a Matter destination' : item.type === 'placement_conflict' || item.type === 'multi_placement_conflict' ? 'Review current filing' : item.type === 'deadline_verification' ? 'Review proposed legal date' : `Review ${reviewFieldLabel(item.field_path)}`
...
function ReviewItemDetail({ detail, tab, onDecisionTab, asOf }: { detail: ReviewDetail; tab: ReviewFilters['tab']; onDecisionTab: () => void; asOf: number }) {
  ...
  const [confirmation, setConfirmation] = useState<ExtractionReviewResolution | ProcessingRecoveryResolution | AmbiguousPlacementResolution | DeadlineReviewResolution | PlacementConflictResolution | MultiPlacementConflictResolution | PossibleDuplicateResolution | null>(null)
  ...
  return <>
    ...
    <div key={tab} id="review-tabpanel" role="tabpanel" aria-labelledby={`review-tab-${tab}`} tabIndex={0} className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4">
      ...
      {tab === 'evidence' ? <>
        ...
        {item.last_decision && item.type!=='possible_duplicate' && <section className="space-y-2"><h3 className="text-sm font-medium">{item.type==='deadline_verification'?'Legal date decision recorded':(item.type==='placement_conflict'||item.type==='multi_placement_conflict') && item.last_decision.action==='keep' ? 'Current filing kept' : (item.type==='placement_conflict'||item.type==='multi_placement_conflict') && item.last_decision.action==='move' ? 'Document moved' : item.last_decision.action === 'select_candidate' ? 'Recorded outcome' : item.last_decision.action === 'continue_manual' ? 'Manual continuation recorded' : item.last_decision.action === 'select_destination' ? 'Placement recorded' : 'Clarification requested'}</h3>{item.type!=='deadline_verification'&&item.type!=='multi_placement_conflict'&&item.last_decision.selected_candidate_id && <p className="break-words text-sm">Selected: {reviewValueLabel(item.evidence.find(e => e.candidate_id === item.last_decision?.selected_candidate_id)?.value)}</p>}{item.last_decision.placement_candidate_id && <p className="break-words text-sm">Destination: {placementValue(item.evidence.find(e => e.candidate_id === item.last_decision?.placement_candidate_id)?.value)?.matter_title}</p>}{item.type==='placement_conflict' && <p className="break-words text-sm">Recorded {item.last_decision.action==='move' ? `Move from ${recordedSource} to ${recordedTarget}` : `Keep in ${recordedSource}`}.</p>}{item.type==='multi_placement_conflict' && <p className="break-words text-sm">Recorded {item.last_decision.action==='move' ? `Move from ${recordedSource} to ${multiCandidateValue(item.evidence.find(e=>e.candidate_id===item.last_decision?.selected_candidate_id)?.value)?.target_matter_title??'the chosen Matter'}` : `Keep in ${recordedSource}`}.</p>}<p className="break-words text-sm">{item.type==='placement_conflict'||item.type==='multi_placement_conflict' ? `Reason: ${item.last_decision.reason}` : item.last_decision.reason}</p></section>}
        {canResolve && item.type==='possible_duplicate' ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide what these {item.evidence.length} documents represent</legend>
