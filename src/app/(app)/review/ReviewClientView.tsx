...
const itemLabel = (item: ReviewQueueItem | ReviewDetail) => item.type === 'possible_duplicate' ? 'Compare two documents' : item.type === 'processing_recovery' ? 'Continue document manually' : item.type === 'ambiguous_placement' ? 'Choose a Matter destination' : item.type === 'placement_conflict' || item.type === 'multi_placement_conflict' ? 'Review current filing' : item.type === 'deadline_verification' ? 'Review proposed legal date' : `Review ${reviewFieldLabel(item.field_path)}`
    ...
    <div key={tab} id="review-tabpanel" role="tabpanel" aria-labelledby={`review-tab-${tab}`} tabIndex={0} className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4">
      ...
      {tab === 'evidence' ? <>
        ...
        {item.type==='possible_duplicate' && <section className="space-y-3 border-t border-[var(--border)] pt-3">
          <h3 className="text-sm font-medium">Exact official key in two current PDFs</h3>
        ...
        {item.type==='possible_duplicate'&&item.last_decision&&<section className="space-y-2 border-t border-[var(--border)] pt-3"><h3 className="text-sm font-medium">{item.last_decision.action==='distinct_documents'?'Distinct documents recorded':'Possible same document recorded'}</h3><p className="break-words text-sm">{item.last_decision.action==='distinct_documents'?'This comparison is closed: the two cited PDFs are recorded as distinct, and the unchanged source pair will not raise Review again.':'This comparison is closed as a finding only. Both cited PDF versions and logical documents remain separate and unchanged; any remediation requires a separate governed Workbench action.'}</p><p className="break-words text-sm">Reason: {item.last_decision.reason}</p></section>}
        ...
        {canResolve && item.type==='possible_duplicate' ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide what these two documents represent</legend>
          <p className="text-caption text-[var(--text-muted)]">Compare both exact Workbench sources first. This decision records your interpretation only. It cannot merge, remove, replace, move, or change effective facts.</p>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="duplicate-outcome" className="mt-1 size-4 accent-[var(--primary)]" checked={choice==='distinct_documents'} onChange={()=>setChoice('distinct_documents')}/><span className="text-sm">Mark as distinct documents<span className="mt-1 block text-caption text-[var(--text-muted)]">A legitimate later document may reuse this reference. The unchanged pair will not be raised again.</span></span></label>
          <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3"><input type="radio" name="duplicate-outcome" className="mt-1 size-4 accent-[var(--primary)]" checked={choice==='possible_same_document'} onChange={()=>setChoice('possible_same_document')}/><span className="text-sm">Mark as possibly the same document<span className="mt-1 block text-caption text-[var(--text-muted)]">Record a follow-up interpretation; both records and PDFs remain intact.</span></span></label>
