...
import { continueProcessingManually, resolveAmbiguousPlacement, resolveExplicitDueDate, resolveExtractionConflict } from '@/lib/actions/review'
import { reviewFieldLabel, reviewValueLabel, taxPeriodComparison, type AmbiguousPlacementResolution, type DeadlineReviewResolution, type ExtractionReviewResolution, type ProcessingRecoveryResolution, type ReviewDetail, type ReviewFilters, type ReviewQueueItem } from '@/lib/review/model'
...
const typeLabel = (type: ReviewQueueItem['type']) => type === 'processing_recovery' ? 'Processing recovery' : type === 'ambiguous_placement' ? 'Placement conflict' : type === 'deadline_verification' ? 'Legal date verification' : 'Extraction conflict'
const itemLabel = (item: ReviewQueueItem | ReviewDetail) => item.type === 'processing_recovery' ? 'Continue document manually' : item.type === 'ambiguous_placement' ? 'Choose a Matter destination' : item.type === 'deadline_verification' ? 'Review proposed legal date' : `Review ${reviewFieldLabel(item.field_path)}`
...
export function ReviewClientView({ queue, detail, filters, asOf }: { queue: Queue; detail: ReviewDetail | null; filters: ReviewFilters; asOf: number }) {
  ...
  const filterControls = <>
    <Filter label="Type" value={filters.type} options={[["all", "All"], ["extraction_conflict", "Extraction conflict"], ["processing_recovery", "Processing recovery"], ["ambiguous_placement", "Placement conflict"], ["deadline_verification", "Legal date verification"]]} onChange={type => navigate({ type, page: undefined })} />
...
function ReviewItemDetail({ detail, tab, onDecisionTab, asOf }: { detail: ReviewDetail; tab: ReviewFilters['tab']; onDecisionTab: () => void; asOf: number }) {
  ...
  const [confirmation, setConfirmation] = useState<ExtractionReviewResolution | ProcessingRecoveryResolution | AmbiguousPlacementResolution | DeadlineReviewResolution | null>(null)
  ...
  return <>
    ...
    <div key={tab} id="review-tabpanel" role="tabpanel" aria-labelledby={`review-tab-${tab}`} tabIndex={0} className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4">
      ...
      {tab === 'evidence' ? <>
        ...
        {canResolve && item.type==='deadline_verification' ? <fieldset className="space-y-3"><legend className="text-sm font-medium">Decide this cited legal date</legend>
