...
test('canonical document route reads active IDs directly and fences Trash with scalar matter context', () => {
  const source = readFileSync(new URL('./page.tsx', import.meta.url), 'utf8')

  assert.match(source, /typeof query\.matterId === 'string'/)
  assert.match(source, /getCanonicalAssignedDocument\(docId, expectedMatterId\)/)
  assert.match(source, /getTrashedDocumentVersionSignedUrl\(exactDocument\.expectedMatterId, docId, doc\.current_version_id\)/)
...
test('canonical document route retains signed PDF, inspector, Notes, timeline detail, and Trash read-only presentation', () => {
  ...
  assert.match(source, /getDocumentVersionSignedUrl\(doc\.current_version_id\)/)
  ...
  assert.match(source, /<PdfViewer url=\{signedDocument\.url\}/)
