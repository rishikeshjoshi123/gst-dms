...
test('canonical document route allowlists source state and preserves exact canonical Trash lineage', () => {
  ...
  assert.match(source, /sourceLocator\.sourceState === 'invalid'[\s\S]*sourceLocator\.versionId \?\? doc\.current_version_id/)
...
test('canonical document route retains signed PDF, inspector, Notes, timeline detail, and Trash read-only presentation', () => {
  ...
  assert.match(source, /<CanonicalDocumentWorkbench[\s\S]*inspectorMetadataByDocumentId=\{inspectorMetadata\}[\s\S]*readOnly=\{isTrashReadOnly\}/)
