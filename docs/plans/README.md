...
A fresh planning task should read `AGENTS.md`, this index, the portfolio status table, and only the plan for the active domain. It must not reconstruct approved decisions from chat history. Organisation Administration remains in progress, but its identity/RBAC foundation is a completed foundation tranche. Document Record and File Lifecycle is in progress: direct-matter and global Inbox canonical upload, placement, recovery, and staged-source verification are complete; the canonical next action is controlled staging transfer followed by separately verified retirement. Platform Operations must ship and pass its gates before rollout beyond the controlled pilot. External Acquisition and Imports remains deliberately deferred until representative spreadsheets are available.
