...
- Automatic layout and temporary dragging preserve enough node, edge and label separation for an unambiguous relationship hit target. Nodes cannot overlap or be moved closer than the measured minimum gap; dragging stops or snaps before violating it. Two relationships may appear visually grouped near a shared endpoint, but their rendered strokes and hit regions never occupy identical geometry: route them in deterministic, closely spaced parallel lanes before they diverge. Multiple canonical types between the same endpoint pair remain the one bundled-edge exception described above.
...
- The complete edge path is the pointer and keyboard interaction target; its label is not the sole target. Hovering/focusing any unique segment emphasizes that full parent-to-child path and its endpoints while sibling paths retain their base treatment. A shared-looking trunk is therefore implemented with distinct parallel hit paths rather than an ambiguous merged stroke or an extra relationship chooser.
- Edge labels are compact semantic surfaces containing the catalogue phrase in sentence case, never uppercase enum text. At low zoom they hide to prevent collisions while the full path remains selectable; keyboard/hover/selection and the relationship inspector expose the phrase. Labels and arrows must remain legible in light/dark and cannot rely on colour alone.
- View is the default read-only graph state. In View, an edge gives restrained hover/focus emphasis and selection opens read-only relationship information. One visibly labelled `Edit timeline` control, supported by an icon, enters a permission-gated editing state; the stable control becomes `Finish editing`. Edit uses stronger whole-path emphasis and contains add, correct and archive operations without exposing separate top-level Add/Correct modes. Edit styling uses semantic emphasis plus a textual mode indication, not colour, glow or canvas decoration alone.
- Suggested relationships do not continuously animate on the ordinary graph. A compact `N relationships to review` warning action opens the matter-scoped Review list; a contextual `Review linkage`/`Review on timeline` action may instead open the graph for one Review item. There is no general Review-mode button on Timeline. The contextual state renders only the selected candidate with dashed warning treatment, highlights its endpoints, subdues unrelated content, exposes only that item's allowed accept/correct/reject actions, and does not unlock general editing. `Cancel`/`Return to Review` leaves the item unresolved.
- Clicking or keyboard-selecting an effective edge opens the Relationships inspector with both documents, canonical direction, display direction, type, evidence, verification, actor/policy, and read-only or permitted edit actions for the current state. Clicking an edge never silently deletes it.
...
The View/Edit, full-path targeting and contextual Review decisions are retained in the [September 13 interaction record](../../decision-history/2026-09-13-matter-timeline-review-and-notes-task-interactions.md).
...
- Do not add a fourth `Needs attention` subview. A fixture-only concept must test whether Overview can carry one conditional compact summary containing the highest-priority item and a count/`View all` action, with details opening their authoritative Task, Deadline or Review surface. Test empty, one-item, mixed, long-content, narrow and mobile states; omit the summary if it competes with document comprehension or creates persistent noise.
...
- Use the approved `matter_identifiers` contract from the Document Hub plan and the [September 11 decision record](../../decision-history/2026-09-11-matter-identity-and-extraction-normalization.md) for external case/proceeding/portal identifiers and their verification/provenance. The workspace represents one independently progressing/challengeable proceeding chain: financial year is an attribute and placement signal, multiple Matters for one client/year remain valid, and one Matter may span multiple periods or financial years.
...
8. **Build relationship review interaction.** Connect the `N relationships to review` action to typed Review and its contextual `Review on timeline` route. Render only the selected candidate in the constrained, cancellable review state and implement accept/correct/reject/archive through the same canonical resolvers as the Review queue, with evidence, stale revision handling, atomic queue closure, Activity, return-position preservation, and no page-local deletion semantics.
...
- Branch fixtures route each parent-to-child relationship through a distinct full-path hit region even where paths appear grouped. Pointer hover and keyboard focus select exactly one relationship without a chooser: View provides restrained complete-path/end-point feedback, while Edit provides stronger feedback and permitted add/correct/archive actions. Node dragging cannot collapse the minimum spacing needed to preserve those targets or visible labels at supported zoom levels.
...
- `Review on timeline` and the Review workspace resolve the same item/revision through the same typed resolver. A successful decision updates the graph and closes the queue item atomically; a concurrent winner returns the current closed result. Contextual review never enables unrelated graph mutations and can be exited without resolving the item.
