...
- **2026-09-01 — Step 3, secured Task reader and transition consumer closure:** migrations `00105`, `00108`, and `00109` made the dedicated Tasks workspace and Notes task summary the live consumers of private Task state. The authenticated Task RPCs derive the exact one active membership from `auth.uid()`, fail closed on invariant corruption, enforce role/tenant/active-resource access, and provide Task list, detail, history, membership context, and optimistic-concurrency transitions. Task transitions append one typed Activity event and projector outbox row with idempotent replay handling; membership changes serialize against Task commands. The workspace replaces legacy action-item state with Task-only current state while retaining the note as immutable origin. Local reset/SQL authority and concurrency fixtures, generated types, TypeScript, migration checks, and independent QA passed. My Work, Activity reading/projecting, Task comments, mention delivery, Review, Notifications, and Today remain deferred.
