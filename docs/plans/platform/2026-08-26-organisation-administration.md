...
- Add organisation-scoped settings under `/settings/organisation/profile`, `/settings/organisation/access`, and `/settings/organisation/operations`. `Access` is the explicit place where Owner/Admin reviews an Associate's baseline role and grants or revokes separately authorised feature access; it must not be hidden inside Document Hub or scattered across feature pages. The exact first-release grant catalogue remains a separate approval decision before this surface is implemented. Owner/Admin can edit authorised fields; other active members receive the small read-only organisation profile only and do not see privileged operational policy or access-management details. Do not expose an organisation Security/MFA tab in the initial release.
...
- Associate feature grants are managed only from Organisation `Access`, use centrally versioned capability keys, and are auditable, revocable, and bounded by the Associate role ceiling. `Manage shared intake` (`intake.manage_shared`) is one required bundled grant: it exposes `All uploads` together with approved standard shared-triage actions, not a view-only global feed. A grant cannot confer Owner/Admin authority, platform operations, quota changes, permanent purge, organisation-policy editing, or access to a resource the Associate otherwise cannot read. The remainder of the first-release grant catalogue still requires approval; no generic arbitrary-permission matrix or free-form capability editor is implemented.
...
7. **Build organisation settings.** Add profile/defaults, the reserved Access section shell, prospective 30/60 departure-notice policy, the Trash plan's 30/60/90 automatic-retention selector, the initial document-placement selector with `manual_suggestions` visibly selected by default, storage entitlement/usage, notification defaults, revision checks, and read-only views for ordinary members. Do not connect Access grant mutations until its first-release capability catalogue and impact design are separately approved. Require an explicit Owner/Admin save before either automatic placement mode becomes effective. Do not add an MFA/security-policy surface in this phase.
