...
- **Read first:** [Current caller gaps](#existing-caller-and-directory-closure) · [Organisation scope](#one-organisation-pilot-and-future-compatibility) · [Authority](#authorisation-transactions-and-service-boundaries).
- **Membership and access:** [Roles](#roles-ownership-and-capabilities) · [Offboarding](#membership-lifecycle-and-offboarding) · [Safe directory](#team-directory-and-member-profiles) · [Invitations](#invitation-lifecycle).
- **Settings and consumers:** [Surfaces](#product-surfaces-and-navigation) · [Organisation policy](#organisation-profile-and-operational-policy) · [Personal settings](#personal-settings) · [UI contract](#ui-layout-and-state-contract).
...
The user explicitly deferred a separate discussion of sole-member departure, empty-organisation retention/deletion and the former member's right to return. The September 14 proposal would archive the organisation, stop ordinary organisation work, and let the former Owner request governed CaseChain recovery through a future Platform Administration surface. Record the unresolved product cases in [the topic note](../../discovery/last-member-and-empty-organisation.md); do not implement the proposal by deleting last-Owner/Admin checks or treating direct database editing as a normal recovery command. This follow-up is not an October signup prerequisite. Its final lifecycle, continuing maintenance/retention jobs, proof-of-return and recovery authority remain unapproved.

### October administration and future Viewer access

- The first production release requires governed invitations, a truthful member directory and immediate Owner/Admin suspension of an ordinary member. Reactivation means an Owner/Admin reverses that suspension and restores the existing member's access without issuing a new invitation; it is useful but not release-critical. Broader role administration, ownership transfer, removal, resignation and complete departure automation may remain deferred behind their existing safeguards.
- When pending invitations exist during onboarding, show them as the primary path. Keep `Create organisation` secondary and explain before confirmation that the current one-organisation rule will prevent accepting another invitation afterwards. An email invitation link authenticates or verifies the matching account and returns to this explicit acceptance step; opening the link never creates membership silently.
- Matter access remains organisation-wide for the October release. The user's post-October direction is for Owner/Admin to choose the exact Matters each Viewer may read. That later contract is recorded in [Viewer Matter access](../../discovery/viewer-matter-access.md); no partial filter or navigation hiding may be presented as access control.
...
- Add organisation-scoped settings under `/settings/organisation/profile` and `/settings/organisation/operations`. There is no first-release Organisation `Access` page or general Associate feature-grant catalogue. Owner/Admin can edit authorised fields; other active members receive the small read-only organisation profile only and do not see privileged operational policy details. Matter-specific internal-expense participants remain inside the owning Financials workspace. Do not expose an organisation Security/MFA tab in the initial release.
...
- Owner/Admin and Associate share the same ordinary legal-work capabilities. Owner/Admin adds application-management authority; it is not a more privileged class of lawyer for routine Matter, document, intake, Task, Note, Review, deadline, or legal-financial work. Viewer remains read-only wherever access is otherwise permitted.
...
- Shared intake is ordinary legal work. Every active Associate, Admin and Owner can use `All uploads` and the approved standard shared-triage actions; there is no `intake.manage_shared` grant. Viewer does not receive unplaced-intake access from the Viewer role.
- The only first-release optional working-access grant is the approved Matter-specific internal-expense permission. Owner/Admin manages participants in Financials; an eligible Associate may receive `view` or `edit`, and a Viewer may receive `view` only. The grant reveals no other Matter or organisation authority and follows the Financials plan's audit, revocation and lifecycle rules.
- Confidential-Matter or ethical-wall restrictions are a parked future consideration. Matter access remains organisation-wide in the first release; do not add a general permission editor, arbitrary capability keys, or per-Matter confidentiality controls without a later approved plan.
...
- A departure notice is a separate case rather than a membership state. Submission keeps the membership active, records the computed effective local date, optional reason and handover note, and shows the user the exact midnight-IST effective time plus whole days remaining. The self-service impact page uses plain user-facing categories: open Tasks and verified legal dates for which the member receives reminders and is expected to follow up. Shared Review items are not owned work and do not appear in offboarding impact; departure only releases any temporary Review claim held by the member. The page explains in everyday language that the optional note gives teammates useful context but does not move work, and states the access-loss and preserved-authorship consequences. Access grants and Owner/Admin coverage appear only when they change this member's outcome; backend-only effects such as digests or pending invitations do not become unexplained checklist items.
...
- Reactivation restores the role that remained valid at suspension time but does not automatically restore Matter-specific internal-expense access that was explicitly revoked during review.
...
- Before suspension, administrative removal, role downgrade, departure submission, or departure execution, return the appropriate impact projection covering open tasks, accountable verified deadlines, incompatible access grants, pending invitations sent by the member, scheduled digests, and Owner/Admin coverage where applicable. Review is a shared authorised queue, so its items are never reassigned during offboarding; any active temporary claim by the departing member is released and the item remains visible in Review. Do not invent a generic `Matter responsibility` handover item: the first-release Matter contract has no canonical lead/owner assignment, and contribution or Matter access must not be presented as operational ownership.
...
- Operations settings own organisation timezone, the prospective 30/60-day departure-notice policy (default 30), default deadline reminder policy, weekly digest defaults, and read-only storage entitlement/usage. Trash retention is owned by the Trash contract: Owner/Admins choose one automatic 30-, 60-, or 90-day period (default 90); there is no manual-only option or separate auto-purge switch. They also own an Owner/Admin-only initial document-placement policy: `manual_suggestions`, `strong_evidence_auto_place`, or `intended_matter_only`. Every new organisation and any safely backfilled organisation without an explicit choice starts on `manual_suggestions`; inferred automatic matter placement is enabled only after an Owner/Admin deliberately selects an automatic mode. The setting describes how unassigned Intake may be placed and can never silently move an already assigned document. Uploading from inside a matter remains a user-declared destination rather than inferred automatic placement.
...
- `Notifications` consumes the approved per-event-family contract: mentions, task assignments, verified deadline reminders, invitation/security events, assigned failures, and exceptional Review escalation directed to authorised administrators. Routine upload/processing completion is not offered because it is not a notification, and shared Review items do not create personal assignment notifications.
...
- Planned departure and immediate administrative removal are separate workflows. Self-service departure uses a dedicated pre-submission impact review and, after submission, a separate countdown/status surface; administrative removal uses a dedicated review workspace rather than a modal, with current impact, explicit work destinations, authority safeguards, and a verb-led confirmation. Neither workflow is compressed into the other or mixed into an ordinary member card.
...
remain required. The Associate working-access policy is resolved: no general
grant catalogue or Organisation Access page is required for the first release.
...
7. **Build organisation settings.** Add profile/defaults, prospective 30/60 departure-notice policy, the Trash plan's 30/60/90 automatic-retention selector, the initial document-placement selector with `manual_suggestions` visibly selected by default, storage entitlement/usage, notification defaults, revision checks, and read-only views for ordinary members. Do not build an Access section or general Associate grant mutations. Require an explicit Owner/Admin save before either automatic placement mode becomes effective. Do not add an MFA/security-policy surface in this phase.
...
- Viewer mutation denial is proved through current client actions and surviving direct write grants/policies, including document columns. Removed/suspended membership and concurrent parent-state changes fail closed. Non-admin member displays cannot recover hidden emails through another feature loader.
...
- Active Associates, Admins and the Owner receive the same ordinary legal-work authority, including `All uploads` and standard shared-intake triage. Tests reject a hidden `intake.manage_shared` dependency or an Organisation Access grant path. Internal-expense access remains separately Matter-scoped and follows the Financials acceptance contract.
...
- A new organisation and a backfilled organisation with no explicit placement choice both resolve to `manual_suggestions`. Associate/Viewer and forged direct-command attempts cannot change it; automatic placement remains disabled until an Owner/Admin explicitly selects and saves an automatic mode, and reverting to manual affects later unassigned Intake without moving already assigned documents.
