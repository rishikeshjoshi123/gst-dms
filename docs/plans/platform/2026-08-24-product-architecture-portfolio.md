...
| Work, Review, Activity, Notifications, and Today | `in-progress` | The approved catalogue inventory, private append-only Activity foundation, and first-class Task write foundation are independently verified. Matter Notes now uses the atomic Task/note command as a live compatibility writer; no Task reader/transition, Activity producer, outbox, Review, Notification, or Today consumer has migrated. | Continue Task step 3 with one coherent secured reader/transition consumer and explicit legacy action-item compatibility, then continue the approved rollout. |
