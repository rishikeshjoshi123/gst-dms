...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, live transactional hierarchy-aware document/matter/client Trash command, exact-PDF duplicate resolution/version-writer fence, tenant-fenced `/trash` workspace, and canonical Client/Matter/Document shared read-only Trash routes are implemented and independently verified. Exact projections are allowlisted and lineage-bound; ordinary collection, dashboard, and Search readers remain typed-active and legacy-compatible active-only. | Implement root-scoped restore with parent/uniqueness validation, conflict resolution, dependent-domain reactivation/re-evaluation, and atomic indexing/schedule events. Keep Permanent Delete authority deferred. |
