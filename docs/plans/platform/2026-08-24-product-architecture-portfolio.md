...
| AI Extraction and Provenance | `in-progress` | Transitional hardening, the scoped structured Vertex failure contract, and immutable source-analysis run/attempt authority are implemented and verified; normalized provenance/model-lifecycle work remains active. | Implement immutable source-field candidate authority with asset/page evidence; defer bindings, effective projections, consumers, embeddings, and UI. |
