...
| Work, Review, Activity, Notifications, and Today | `in-progress` | The approved catalogue inventory, private append-only Activity foundation, and first-class Task write foundation are independently verified. Matter Notes now uses the atomic Task/note command as a live compatibility writer: action-item creation atomically emits one typed `task.created` Activity event and projector outbox row, and due-dated Tasks use the private organisation timezone rather than a personal profile timezone. No Task reader/transition, Activity reader/projector worker, Review, Notification, or Today consumer has migrated. | Continue Task step 3 with one coherent secured reader/transition consumer and explicit legacy action-item compatibility, then continue the approved rollout. |
...
| Platform Operations | `approved` | The private platform-operator identity/capability, AAL2-bound single-use privileged-intent, durable service-ingested alert, immutable model/runtime/pricing, append-only provider-usage ledger, and one live service-only document-extraction usage writer are independently verified. The writer derives its complete accounting identity from a terminal provenance run and reconciles terminal replay without affecting legal-domain state. There is still no bootstrapped operator, browser grant, legacy usage cutover, secured usage consumer/rollup, or console. Platform Operations must still ship and pass its gates before rollout beyond the controlled pilot. | Continue the approved secured usage consumer/rollup foundation, then complete the backup/restore gate before broader rollout. |
