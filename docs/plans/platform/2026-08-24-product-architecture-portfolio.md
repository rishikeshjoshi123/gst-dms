...
| Document Record and File Lifecycle | `in-progress` | Direct-matter and global Inbox canonical upload, placement, authorised preview, recovery, staged-source verification, controlled non-destructive transfer, aggregate-safe retirement evidence inventory, durable dispatch, and explicit scoped reprocess intent are implemented and verified. A scoped worker and a separate fail-closed staged-source purge remain pending. | Implement scoped-worker completion/failure fencing, Vertex failure handling, and outbox retention/compaction; then implement staged-source purge as a separate tranche before adapter contract removal. |
...
| AI Extraction and Provenance | `in-progress` | Transitional hardening exists; normalized provenance/model-lifecycle work remains active. Structured Vertex output, bounded failure recovery, and append-only attempt evidence are now decision-complete. | Implement the approved Vertex failure contract with the durable-processing tranche, then continue normalized provenance/model-lifecycle cut-over. |
...
All final approval passes, including Platform Operations, are complete; there are no remaining proposed-plan approval passes. Organisation Administration remains in progress, but its identity/RBAC foundation, canonical membership RLS cutover, hash-only invitation commands, safe acceptance flow, and associated local database acceptance gates are stable as a completed foundation tranche. The Document Record and File Lifecycle plan is now in progress: direct-matter and global Inbox canonical upload, placement, recovery, staged-source verification, controlled transfer, non-destructive retirement evidence inventory, durable outbox authority, trusted wake-up, bounded organisation-aware draining, and explicit scoped reprocess intent are complete and verified. The approved canonical next action is implementation and independent verification of scoped-worker completion/failure fencing, outbox retention/compaction, and structured Vertex failure handling. The staged retention decision is now also decision-complete: after durable processing is checkpointed, a separate fail-closed recurring job may purge only freshly verified, assigned, redundant staging PDFs, leave exceptional sources for retry/recovery, and write content-free tombstones; adapter removal remains a later verified contract step. Keep External Acquisition and Imports deliberately deferred until representative spreadsheets are available, and allow implementation work to consume only approved contracts and their dependencies. Platform Operations must ship and pass its gates before rollout beyond the controlled pilot.
