...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, transactional hierarchy commands, exact-PDF/version-writer fence, tenant-fenced workspace, canonical shared read-only routes, and root-scoped Restore with fenced search/schedule effects are implemented and independently verified. Exact projections are allowlisted and lineage-bound; ordinary collection, dashboard, and Search readers remain typed-active and legacy-compatible active-only. | Repair the pre-existing `trash.operation_created.v1` durable-outbox dispatch gap, then resume the approved retention-settings and prospective-policy-snapshot tranche. Keep Permanent Delete authority deferred. |
