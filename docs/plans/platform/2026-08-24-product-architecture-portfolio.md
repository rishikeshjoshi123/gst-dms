...
| Work, Review, Activity, Notifications, and Today | `in-progress` | The approved catalogue inventory and private append-only Activity definition/event/projector foundation are implemented and independently verified; no legacy Activity producer, reader, UI, Task, Review, Notification, or Today consumer has migrated yet. | Introduce first-class Tasks through one coherent live command/read caller, then continue the approved rollout. |
