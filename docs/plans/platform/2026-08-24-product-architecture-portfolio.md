...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, live transactional hierarchy-aware document/matter/client Trash command, exact-PDF duplicate resolution/version-writer fence, and tenant/lineage-fenced exact-resource read context are implemented and independently verified. Normal collection, dashboard, and Search readers require typed active state plus legacy compatibility state; exact routes remain intentionally closed for Trash until the later read-only UI/action-suppression tranche. | Build the approved `/trash` workspace before restore or purge work. |
