...
| AI Extraction and Provenance | `in-progress` | Transitional hardening and the scoped structured Vertex failure contract are implemented; normalized provenance/model-lifecycle work remains active. | Continue normalized provenance/model-lifecycle cut-over after the document lifecycle compaction checkpoint. |
