...
| AI Extraction and Provenance | `in-progress` | Transitional hardening, immutable source runs/candidates, version bindings, effective decisions, processing provenance writes, and the bounded effective-metadata Search consumer fence are implemented and verified; normalized provenance/model-lifecycle work remains active. | Implement the next high-use effective-metadata consumer, beginning with inspector/assignment reads. Keep multi-index rollout and page-aware Search as separate later tranches. |
