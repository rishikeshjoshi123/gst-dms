...
| Platform Operations | `approved` | The private platform-operator identity/capability prerequisite is independently verified; it intentionally contains no bootstraped operator or live console. Platform Operations must still ship and pass its gates before rollout beyond the controlled pilot. | Continue the approved trust/config/accounting/audit/alert foundation, then complete the backup/restore gate before broader rollout. |
