...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, live transactional hierarchy-aware document/matter/client Trash command, and exact-PDF duplicate resolution/version-writer fence are implemented and independently verified. Inbox resolves active/history/Intake duplicates without disclosing Trash/restricted identity; all production version writers share organisation/SHA and idempotency fences. | Add trash-aware exact-resource loaders and read-only capability context before the Trash route, restore, or purge work. |
