...
| Universal Search and Evidence Retrieval | `in-progress` | The versioned offline relevance/security baseline and typed embedding-provider migration are independently verified. The provider now serves the real semantic query, leased search-index worker, and matter reindex callers with explicit corpus/query intent and documented Vertex response validation. No rebuilt Search storage, hybrid retrieval, or Search workspace has migrated. | Continue the approved private search-item/chunk storage and one real indexing-writer slice; preserve tenant, document-version, Trash, and replay fences. |
| Work, Review, Activity, Notifications, and Today | `in-progress` | The approved catalogue inventory, private append-only Activity foundation, and first-class Task write foundation are independently verified. Matter Notes now uses the atomic Task/note command as a live compatibility writer: action-item creation atomically emits one typed `task.created` Activity event and projector outbox row. No Task reader/transition, Activity reader/projector worker, Review, Notification, or Today consumer has migrated. | Continue Task step 3 with one coherent secured reader/transition consumer and explicit legacy action-item compatibility, then continue the approved rollout. |
