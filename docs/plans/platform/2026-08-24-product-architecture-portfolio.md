...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, transactional hierarchy commands, exact-PDF/version-writer fence, tenant-fenced workspace, canonical shared read-only routes, root-scoped Restore, typed creation-time semantic-search invalidation, live 30/60/90-day prospective retention policy with scheduled-deletion Team attention, and governed permanent delete are implemented and independently verified. The authenticated local-browser gate confirmed durable queueing, terminal purge/tombstone state, and the empty post-purge workspace using a disposable Owner fixture. Exact projections are allowlisted and lineage-bound; ordinary collection, dashboard, and Search readers remain typed-active and legacy-compatible active-only. There is no production legacy-data migration: the database contains test data only and first production use starts with automatic retention. | No further Trash action is queued. Continue the approved Work foundation when its selected-organisation authority blocker is resolved. |
| AI Extraction and Provenance | `in-progress` | Transitional hardening, immutable source runs/candidates, version bindings, effective decisions, processing provenance writes, the bounded effective-metadata Search consumer fence, live inspector/relationship consumers, processing-time relationship placement, and the Matter Wiki effective-context consumer are implemented and verified; no further connected raw-metadata consumer exists in the current runtime map. | Retain automatic assignment as a prerequisite until a real caller exists; continue its named model-lifecycle/search rollout work only at the corresponding later integration stages. Resume the portfolio at Hierarchical Resource Trash. |
| Universal Search and Evidence Retrieval | `in-progress` | The relevance/security baseline, typed embedding provider, and private metadata-item/index-run writer are independently verified. Existing leased indexing/reindex writers now maintain one force-RLS metadata projection with version, projection, effective-value, Matter/client, Trash, and replay fences. No page-aware chunks, hybrid query, or Search workspace has migrated. | Continue page-aware text/OCR chunking and changed-chunk writes, preserving source-version/page anchor, tenant, Trash, lease, and replay fences. |
| Work, Review, Activity, Notifications, and Today | `in-progress` | The approved catalogue inventory, private append-only Activity foundation, first-class Task write foundation, secured Task reader/transition consumer, one-to-one legacy action-item backfill, and live Task Comments closure are independently verified. Matter Notes is a compatibility writer; the dedicated Tasks workspace owns current Task state and task-scoped conversation. Exact-one-active membership, tenant/RLS/CAS/replay, membership/Trash serialization, typed Activity/projector-outbox events, explicit backfill dispositions, terminal commentability, and suspended/inaccessible read-only fences are in place. Activity reader/projector, Review, Notifications, My Work, and Today remain deferred. | Obtain visual approval for the dedicated Review workspace concept, then implement its smallest secure producer/consumer closure; continue independent approved Organisation Administration work in parallel. |
| Document Hub and Workbench | `approved` | The canonical `/documents` collection, `/documents/{documentId}` reader, and secured `/documents` queue → details → source presentation are verified live closures over organisation-fenced Intake and assigned-document projections; legacy Inbox and nested Matter routes are compatibility redirects. | Add a shared version/source-locator consumer only when its secured loader contract is ready; retain the verified canonical collection/document routes and existing actions. |
...
| Organisation Administration | `in-progress` | The identity/RBAC foundation, canonical membership RLS cutover, hash-only invitation commands, safe acceptance flow, and associated local database acceptance gates are stable as a completed foundation tranche. The one-active-organisation RPC authority, portable professional profile boundary, self-only contribution projection, governed 30/60-day departure-notice contract, immutable post-policy membership snapshots, and private membership-bound departure-case history are independently verified; remaining implementation stays in progress. | Harden membership `org_id`/`user_id` identity against post-insert drift before any update-capable departure command, then resume typed departure/early-release execution, midnight-IST recovery, secured profile/contribution projections, and the approved Team/My settings surfaces. |
| Platform Operations | `approved` | The private platform-operator identity/capability, AAL2-bound single-use privileged-intent, durable service-ingested alert, immutable model/runtime/pricing, append-only provider-usage ledger, live service-only document-extraction usage writer, and private UTC daily rollup consumer are independently verified. The writer derives its complete accounting identity from a terminal provenance run and reconciles terminal replay without affecting legal-domain state; its deferred ledger trigger maintains quality-preserving, unknown-cost-safe aggregates without a browser or service-role read grant. There is still no bootstrapped operator, browser grant, legacy usage cutover, secured platform reader, or console. Platform Operations must still ship and pass its gates before rollout beyond the controlled pilot. | Complete the approved backup/restore gate before broader rollout. |
