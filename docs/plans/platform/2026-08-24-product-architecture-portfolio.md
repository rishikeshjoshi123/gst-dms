...
- Workers consume durable work, use idempotency keys, append attempt/run records, and can retry without duplicating records, notifications, links, deadlines, or financial events.
...
| Document Record and File Lifecycle | `in-progress` | Direct-matter and global Inbox canonical upload are implemented and verified, including durable validation, version-authorised access, quota/tombstone controls, and conservative recovery. | Implement canonical Inbox placement, assignment, discard, and preview; eliminate staging-bucket copy on the canonical path before resumable legacy backfill. |
...
- `OutboxEvent`: event kind/version, aggregate ID, tenant, idempotency key, payload, attempts, next attempt, and delivery state.
