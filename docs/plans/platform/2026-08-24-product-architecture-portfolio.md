...
| AI Extraction and Provenance | `in-progress` | Transitional hardening, the scoped structured Vertex failure contract, immutable source-analysis run/attempt authority, and immutable asset/page-evidenced source-field candidates are implemented and verified; normalized provenance/model-lifecycle work remains active. | Implement document-version bindings and document-level candidate materialization without a duplicate model call; defer effective projections, consumers, embeddings, and UI. |
