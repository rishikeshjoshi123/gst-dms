...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation and the live, transactional hierarchy-aware document/matter/client Trash command are implemented and independently verified. Existing delete callers now use the authenticated command with actor-scoped idempotency, tenant/capability checks, active-descendant trees, direct-write denial, and local concurrency proof. | Extend organisation-local exact-PDF duplicate resolution across active, historical, Intake, and Trash references before restore, purge, or Trash UI work. |
