...
| AI Extraction and Provenance | `in-progress` | Transitional hardening, immutable source runs/candidates, version bindings, effective decisions, processing provenance writes, the bounded effective-metadata Search consumer fence, and the live inspector, relationship-dialog, and relationship re-evaluation consumers are implemented and verified; normalized provenance/model-lifecycle work remains active. | Migrate the processing-time `placeDocument` relationship consumer. Keep automatic assignment as a prerequisite until a real caller exists, and keep multi-index rollout and page-aware Search separate. |
