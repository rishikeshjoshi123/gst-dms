...
| Hierarchical Resource Trash | `in-progress` | The private trash-operation, membership, retention, hold, typed-resource-state, and authority foundation is implemented and independently verified; no live trash consumer has been claimed yet. | Implement transactional hierarchy-aware trash commands for document, matter, and client before restore, purge, or Trash UI work. |
