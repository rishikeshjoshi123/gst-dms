...
| Universal Search and Evidence Retrieval | `in-progress` | The versioned offline relevance and security baseline is independently verified: 112 synthetic cases cover planned retrieval modes, exact paise boundaries, availability, adversarial inputs, and fixture-level tenant non-disclosure; it also models the limited legacy search behavior separately from target expectations. No runtime Search consumer, provider, schema, or UI has migrated. | Continue the approved typed embedding-provider contract with its real existing corpus/query callers; do not claim hybrid retrieval or a Search workspace. |
