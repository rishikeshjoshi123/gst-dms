...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, live transactional hierarchy-aware document/matter/client Trash command, exact-PDF duplicate resolution/version-writer fence, exact-resource context, and tenant-fenced root-only `/trash` workspace are implemented and independently verified. Normal collection, dashboard, and Search readers require typed active state plus legacy compatibility state; exact routes and `/trash` item links remain intentionally closed for Trash until the later read-only UI/action-suppression tranche. | Add the persistent read-only Trash context and suppress mutations across Client, Matter, and Document routes; only then enable safe `/trash` item links. |
