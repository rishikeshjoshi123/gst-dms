...
| Platform Operations | `approved` | The private platform-operator identity/capability and AAL2-bound, single-use privileged-intent prerequisites are independently verified; they intentionally contain no bootstrapped operator, browser grant, live command, or console. Platform Operations must still ship and pass its gates before rollout beyond the controlled pilot. | Continue the approved trust/config/accounting/audit/alert foundation, then complete the backup/restore gate before broader rollout. |
