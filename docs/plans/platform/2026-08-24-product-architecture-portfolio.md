...
| Hierarchical Resource Trash | `in-progress` | The private Trash foundation, transactional hierarchy commands, exact-PDF/version-writer fence, tenant-fenced workspace, canonical shared read-only routes, root-scoped Restore, typed creation-time semantic-search invalidation, and live 30/60/90-day prospective retention policy with scheduled-deletion Team attention are implemented and independently verified. Exact projections are allowlisted and lineage-bound; ordinary collection, dashboard, and Search readers remain typed-active and legacy-compatible active-only. | Implement the approved root-operation permanent-delete impact and durable execution workflow, including holds/blockers, confirmation, dependency cleanup, tombstones, retry/reconciliation, and operations visibility. |
