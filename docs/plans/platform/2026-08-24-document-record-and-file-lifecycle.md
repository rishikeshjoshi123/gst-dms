...
Keep legacy reads and assignment behind their compatibility/fence contracts. Any adapter contract removal remains a separately approved migration and rollout checkpoint and may begin only when the zero-unresolved retirement evidence contract is satisfied; no separate staging attestation for legal hold, export, backup, or retention is required. The completed staged-source purge is not authority to remove a legacy row, expose a new read path, or alter assignment behavior. The direct document/reprocess Trigger bypasses remain removed; canonical upload completion and the approved search-index reprocess command send only a trusted, content-free, coalesced, non-blocking singleton dispatcher wake after their outbox transaction commits, while the one-minute recovery remains authoritative if that wake fails.
