...
Implement and independently verify the remaining durable-processing write-path tranche: add explicit scoped reprocess authority, idempotent version-scoped processing completion/recovery, the approved Vertex controlled-failure contract, and privileged outbox compaction. The direct document/reprocess Trigger bypasses have been removed; canonical upload completion now sends only a trusted, content-free, coalesced, non-blocking singleton dispatcher wake after its outbox transaction commits, while the one-minute recovery remains authoritative if that wake fails. Keep legacy reads and assignment behind their compatibility/fence contracts; do not implement the separately approved staged-source purge or adapter contract removal inside this tranche. After durable processing is checkpointed, implement the fail-closed staged-source purge as its own migration/worker/QA tranche before any adapter removal.
