...
Implement and independently verify privileged outbox retention/compaction. The direct document/reprocess Trigger bypasses have been removed; canonical upload completion and the approved search-index reprocess command now send only a trusted, content-free, coalesced, non-blocking singleton dispatcher wake after their outbox transaction commits, while the one-minute recovery remains authoritative if that wake fails. Keep legacy reads and assignment behind their compatibility/fence contracts; do not implement the separately approved staged-source purge or adapter contract removal inside this tranche. After compaction is checkpointed, implement the fail-closed staged-source purge as its own migration/worker/QA tranche before any adapter removal.
