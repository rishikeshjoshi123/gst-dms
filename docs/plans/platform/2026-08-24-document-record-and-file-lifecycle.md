...
- Added migrations `00059` and `00060` with service-only, organisation-scoped purge claims for completed transfers that have a fresh equal retirement audit. Every claim, grant, intent, and confirmation requires positive, current coverage of legal-hold, export, backup, recovery, and retention-lock guards; each organisation starts `unknown`, so an empty blocker ledger can never authorise deletion. A PostgreSQL owner must separately verify and make a short-lived `enforced` or `disabled_verified` operational attestation for every guard; the migration records that procedure but makes no claim that any attestation has occurred.
- The authoritative predicate requires the legacy source to remain assigned to an active matter and the canonical asset to be the exact asset of the assigned document’s current, valid, immutable version in that same matter and organisation. It rechecks source lineage, audit and transfer facts, active legacy/backfill/audit/purge work, all running analysis or processing work (including null-lease and expired-lease rows), scoped organisation/asset/source blockers, and open canonical recovery cases before exposing either Storage key.
...
- Contradictory source/canonical observations and database inconsistency create a durable, content-free recovery fence. A future hold/export/backup/recovery/retention-lock writer is scoped to organisation, canonical asset, or source and must call a service-only command that serializes against the mapped source: before intent it records the blocker and intent fails; after `delete_intended` it returns `purge_in_progress` without recording a late blocker or starting an external safety action. Direct privileged repair writes retain a recovery fence. The implementation neither deletes a legacy row nor changes compatibility reads, assignment, adapter fences, or canonical asset retention.
- Clean local migration replay, rollback-only SQL acceptance for unknown coverage, owner-only attestation, immutable assigned-version lineage, null-lease running work, scoped late-block recovery, isolation and content-free reports; focused worker tests; generated database types; TypeScript checking; migration validation; schema diff; and whitespace checks passed. Schema lint retains only the pre-existing unused-variable warning in `claim_staged_document_backfill_batch`.
