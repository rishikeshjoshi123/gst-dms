...
- An unassigned Inbox/Intake source is never auto-purged. It remains available for human assignment or an explicit human discard action, regardless of how long it has waited.
- A separate service-only, bounded, recurring purge job owns cleanup only after assignment. It may claim only an explicitly mapped staging source whose Intake has been assigned to a matter and whose expected canonical asset is referenced by that document's immutable current version.
- Before deletion, the job must freshly prove organisation/path lineage; source and destination reachability; exact byte-size and SHA-256 equality; readable validated PDF state; the expected asset–Intake–document/version relationship; and the absence of active upload, assignment, verification, transfer, audit, processing, or staging-specific recovery work. A verified same-organisation duplicate may substitute for the copied destination only after the same equality, reachability, assignment, and recovery checks pass.
- Legal hold, export, backup, and retention policy apply to the canonical matter document and its hierarchical Trash lifecycle, not to a verified redundant staging copy. Matter exports and archival backups read canonical document versions. A legal hold or retention rule prevents canonical Trash purge; it does not preserve a second transport copy after canonical equality is proved.
...
### Completed: fail-closed staged-source purge and tombstone authority (2026-08-29)

- Added migrations `00059` through `00061` with service-only, organisation-scoped purge claims for completed transfers that have a fresh equal retirement audit. Migration `00061` records the approved ownership boundary: unassigned Intake is manual-only, while a staging object becomes purgeable only after exact assignment and canonical-version equality make it a redundant transport copy.
- The authoritative predicate requires the legacy source to remain assigned to an active matter and the canonical asset to be the exact asset of the assigned document’s current, valid, immutable version in that same matter and organisation. It rechecks source lineage, audit and transfer facts, active legacy/backfill/audit/purge work, all running analysis or processing work (including null-lease and expired-lease rows), staging-specific recovery blockers, and open canonical recovery cases before exposing either Storage key.
- The recurring Trigger maintenance job accepts no task payload. It independently downloads and parses both source and canonical PDF, proves actual byte/hash equality, records a leased durable intent, obtains a second fresh deletion grant, removes only the granted legacy staging object, and writes a minimal content-free confirmation tombstone. A trusted post-intent 404 reconciles a lost Storage response; transient failure records no terminal result.
- Contradictory source/canonical observations and database inconsistency create a durable, content-free recovery fence. Recovery writers remain scoped to organisation, canonical asset, or source and serialize against the mapped source. Legal hold, export, backup, and retention are enforced on the canonical matter document and Trash hierarchy instead of preserving a redundant staging copy. The implementation neither deletes a legacy row nor changes compatibility reads, assignment, adapter fences, or canonical asset retention.
- Clean local migration replay, rollback-only SQL acceptance for manual-only unassigned Intake, immutable assigned-version lineage, exact hash/byte equality, null-lease running work, recovery fencing, canonical-policy separation, isolation and content-free reports; focused worker tests; generated database types; TypeScript checking; migration validation; schema diff; and whitespace checks passed. Schema lint retains only pre-existing warnings outside this tranche.
...
### Completed: zero-unresolved staging compatibility cutover (2026-08-29)

- Migration `00062` invokes one database-authoritative, fail-closed aggregate gate before altering any legacy access contract. It rejects every relevant organisation with unresolved retirement reports, unconfirmed source-purge tombstones, active or queued canonical work (including lease uncertainty), pending or leased lifecycle outbox delivery for a mapped canonical asset/Intake/document/version—including `intake.assigned.v1` and `intake.discarded.v1`—recovery cases, staging recovery blockers, access/lineage diagnostics, or unproven quarantined assets.
- Once the gate passes, the browser cannot read or mutate `staged_documents` or the legacy staging bucket, and `staged_documents` is removed from the realtime publication. Legacy app read/action helpers remain non-authorising `adapter_retired` tombstones with no storage locator; service-only backfill, transfer, audit, and purge RPC execution is revoked. The retained service-readable retirement reports preserve their gate-proven adapter-fence result without calling the revoked source-ID fence helper. Legacy rows, mappings, audits, and content-free purge tombstones remain preserved as historical evidence.
- Inbox, Review, notifications, and signed-read consumers now project canonical Intake and immutable document/version authority only. Historical staged notifications remain readable as retired audit history with no Inbox action. The legacy staged realtime channel, staging assignment/discard/auto-create actions, Review slice, and staged-source scheduled workers are removed. Unassigned canonical Intake remains available solely for explicit human placement or discard.
- No staging-specific legal-hold, export, backup, or retention attestation was added: those controls continue to govern the canonical matter document and its hierarchical Trash lifecycle.

### Canonical next action

The staging compatibility adapter and legacy assignment path are retired. Retain the historical staging tables, backfill mappings, retirement audits, and content-free tombstones unless a later separately approved data-retention contract authorises their removal. The direct document/reprocess Trigger bypasses remain removed; canonical upload completion and the approved search-index reprocess command send only a trusted, content-free, coalesced, non-blocking singleton dispatcher wake after their outbox transaction commits, while the one-minute recovery remains authoritative if that wake fails.
