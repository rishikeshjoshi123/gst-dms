...
Implement and independently verify the remaining durable-processing write-path tranche: replace pre-existing direct document/reprocess Trigger calls with a trusted, coalesced, non-blocking post-commit outbox wake carrying only a safe event/batch reference; then add explicit scoped reprocess authority, idempotent version-scoped processing completion/recovery, the approved Vertex controlled-failure contract, and privileged outbox compaction. Keep legacy reads and assignment behind their compatibility/fence contracts; do not implement the separately approved staged-source purge or adapter contract removal inside this tranche. After durable processing is checkpointed, implement the fail-closed staged-source purge as its own migration/worker/QA tranche before any adapter removal.
