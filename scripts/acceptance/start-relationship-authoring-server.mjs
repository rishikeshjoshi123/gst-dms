...
if (!workdir || !readFileSync(join(workdir, 'supabase/config.toml'), 'utf8').includes('project_id = "dms-relationship-authoring-155"')) throw new Error('Supply the exclusively owned dms-relationship-authoring-155 acceptance workdir.')
...
if (status.status !== 0) throw new Error('Isolated Relationship authoring database is unavailable.')
const local = JSON.parse(status.stdout)
if (local.API_URL !== 'http://127.0.0.1:57321' || new URL(local.DB_URL).port !== '57322') throw new Error('Unexpected Relationship authoring acceptance destination.')
const workspace = mkdtempSync(join(tmpdir(), 'dms-relationship-authoring-app-'))
...
const env = {
  ...process.env, NEXT_PUBLIC_APP_URL: 'http://127.0.0.1:3105', NEXT_PUBLIC_SUPABASE_URL: local.API_URL,
...
const args = build ? ['build', '--webpack'] : ['start', '--hostname', '127.0.0.1', '--port', '3105']
console.log(`Relationship authoring ${build ? 'build' : 'browser server'} uses isolated application snapshot ${workspace} and local API 57321.`)
