...
if (!workdir || !readFileSync(join(workdir, 'supabase/config.toml'), 'utf8').includes('project_id = "dms-review-153"')) throw new Error('Supply the exclusively owned dms-review-153 acceptance workdir.')
...
if (status.status !== 0) throw new Error('Isolated Review database is unavailable.')
const local = JSON.parse(status.stdout)
if (local.API_URL !== 'http://127.0.0.1:55321' || new URL(local.DB_URL).port !== '55322') throw new Error('Unexpected Review acceptance destination.')
const workspace = mkdtempSync(join(tmpdir(), 'dms-review-app-'))
...
const env = {
  ...process.env, NEXT_PUBLIC_APP_URL: 'http://127.0.0.1:3103', NEXT_PUBLIC_SUPABASE_URL: local.API_URL,
...
const args = build ? ['build', '--webpack'] : ['start', '--hostname', '127.0.0.1', '--port', '3103']
console.log(`Review ${build ? 'build' : 'browser server'} uses isolated application snapshot ${workspace} and local API 55321.`)
